...
function showQrcode(){
    ...
    $('.js-ewmtab li').on('click', function () {
        $('.ewm-thumbnail').attr("src","/app/themes/vcard02/static/images/noimage.jpg");
        var _this = $(this);
...
function showWxQrcode() {
    $('.js-quickmenu').find('a').each(function(){
        if ($(this).attr('href')) {
            //微赞不支持短信发送
            if ($(this).attr('href').indexOf('unsafe:sms') > -1) {
                $(this).attr('href', $(this).attr('href').replace('unsafe:sms','sms'));
            }
            //如果自定义的菜单中设置的连接是###则打开个人微信名片
            if ($(this).attr('href') == "###") {
                $(this).attr('href', "javascript:menuShowWxQrcode()");
            }
        }
    });
