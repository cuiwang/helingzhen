...
if($_W['ispost']) {
    ...
    if($pay_type == "baifubao") {
        ...
        $return_url = $_W['siteroot']."web/source/shop/common/return_url.inc.php";//后台通知地址
        $page_url = $_W['siteroot']."web/source/shop/common/return_url.inc.php";//前台通知地址
        ...
        message("创建订单失败，请选择其他支付方式",url('shop/recharge'));
    }else{
        /**************************请求参数**************************/
        //支付类型
        $payment_type = "1";
        //必填，不能修改
        //服务器异步通知页面路径
        $notify_url = $_W['siteroot']."web/source/shop/notify_url.ctrl.php";
        //需http://格式的完整路径，不能加?id=123这类自定义参数

        //页面跳转同步通知页面路径
        $return_url =  $_W['siteroot']."web/source/shop/notify_url.ctrl.php";
