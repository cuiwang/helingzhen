...
if ($do == 'forward') {
	...
	if($_GPC['type']==2){
		$result = $account_platform->getAuthInfo($_GPC['auth_code']);
		$update=array(
			'access_token'=>$result['authorization_info']['authorizer_access_token'],
			'expirein'=>TIMESTAMP+$result['authorization_info']['expires_in']-200,
			'refresh_token'=>$result['authorization_info']['authorizer_refresh_token']
		);
		pdo_update('account_wxapp',$update,array('key'=>$result['authorization_info']['authorizer_appid']));
		//上传代码
		$account_platform->build($result['authorization_info']['authorizer_appid']);
		itoast('授权上传代码成功,已进入审核状态！', url('wxapp/version/front_download'), 'success');
