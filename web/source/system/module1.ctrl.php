...
if ($do == 'recycle') {
	...
	if ($operate == 'delete') {
		...
		message('模块已放入回收站', url('system/module/prepared', array('status' => 'recycle')), 'success');
	} elseif ($operate == 'recover') {
		pdo_delete('modules_recycle', array('modulename' => $name));
		message('模块恢复成功', url('system/module/install', array('m' => $name)), 'success');
	}
	template('system/module1');
}
if($do == 'batch-install') {
...
if($do == 'permission') {
	...
	if (empty($module)) {
		message('你访问的模块不存在. 或许你愿意去系统云服务平台看看. ', ADDONS_URL.'/web/index.php?keyword=' . $_GPC['title']);
	}
...
if($do == 'install') {
	...
	if (!empty($manifest)) {
		$r = cloud_m_prepare($modulename);
		if (is_error($r)) {
			message($r['message'], url('system/module/prepared'), 'error');
		}
	}
	...
	if (pdo_insert('modules', $module)) {
		...
		if (empty($module_subscribe_success)) {
			message('模块安装成功, 请按照【公众号服务套餐】【用户组】来分配权限！模块订阅消息有错误，系统已禁用该模块的订阅消息，详细信息请查看 <div><a class="btn btn-primary" style="width:80px;" href="' . url('system/subscribe/subscribe') . '">订阅管理</a> &nbsp;&nbsp;<a class="btn btn-default" href="' . url('system/module1') . '">返回模块列表</a></div>', '', 'tips');
		} else {
			message('模块安装成功, 请按照【公众号服务套餐】【用户组】来分配权限！', url('system/module1'), 'success');
		}
	} else {
...
if ($do == 'uninstall') {
	...
	if ($module['isrulefields'] && !isset($_GPC['confirm'])) {
		message('卸载模块时同时删除规则数据吗, 删除规则数据将同时删除相关规则的统计分析数据？<div><a class="btn btn-primary" style="width:80px;" href="' . url('system/module/uninstall', array('id' => $_GPC['id'], 'confirm' => 1)) . '">是</a> &nbsp;&nbsp;<a class="btn btn-default" style="width:80px;" href="' . url('system/module/uninstall', array('id' => $_GPC['id'], 'confirm' => 0)) . '">否</a></div>', '', 'tips');
	} else {
		...
		message('模块已放入回收站！', url('system/module1'), 'success');
	}
}
...
if($do == 'upgrade') {
	...
	if(strrchr($manifest['install'],'.')=='.php'){
		message('备用更新异常，请移步系统默认更新接口更新该模块！',url('system/module'));
	}
	...
	//	message('模块更新成功！ <br> 系统检测到您的模块目录存在异常文件！请确认是否是自己二次开发所添加的文件.<div><a class="btn btn-primary" href="' . url('system/filecheck/checkmodule') . '">现在查看</a>&nbsp;&nbsp;&nbsp;<a class="btn btn-default" href="' . url('system/module1/') . '">返回模块列表</a></div>', '', 'success');
	//}
	message('模块更新成功！ <br> 由于数据库更新, 可能会产生多余的字段. 你可以按照需要删除.<div><a class="btn btn-primary" href="' . url('system/database/trim') . '">现在去删除</a>&nbsp;&nbsp;&nbsp;<a class="btn btn-default" href="' . url('system/module1/') . '">返回模块列表</a></div>', '', 'success');
}
