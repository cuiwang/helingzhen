...
if ($do == 'bak') {
	$name=trim($_GPC['id']);
	$mdir=IA_ROOT .'/data/wxappsbak/'.$name;
	if(!is_dir($mdir)){
		message('没有可回滚的小程序版本，或者上次更新没有修改文件，无需回滚！');
	}
	$time=0;
	$bak = '';
	...
	pdo_update('wxapps',array('version'=>$version),array('name'=>$name));
	message('成功将小程序版本回滚至上次更新前的版本！');
}
if ($do == 'recycle') {
	$operate = $_GPC['op'];
	$name = trim($_GPC['name']);
	if ($operate == 'delete') {
		pdo_insert('wxapps_recycle', array('modulename' => $name));
		message('小程序已放入回收站', url('system/wxapp/prepared', array('status' => 'recycle')), 'success');
	} elseif ($operate == 'recover') {
		pdo_delete('wxapps_recycle', array('modulename' => $name));
		message('小程序恢复成功', url('system/wxapp/install', array('m' => $name)), 'success');
	}
	template('system/wxapp');
}
if($do == 'batch-install') {
	if(empty($_W['isfounder'])) {
		message('您没有安装小程序的权限', '', 'error');
	}
	
	if($_W['ispost']) {
		$modulename = $_GPC['m_name'];
		if(pdo_fetchcolumn("SELECT mid FROM " . tablename('wxapps') . " WHERE name = :name", array(':name' => $modulename))) {
			exit('error');
		}
		...
		if(pdo_fetchcolumn("SELECT mid FROM " . tablename('wxapps') . " WHERE name = :name", array(':name' => $manifest['application']['identifie']))) {
			exit('error');
		}
...
if($do == 'info') {
	$m = trim($_GPC['m']);
	if($_W['isajax']) {
		$data = pdo_fetch('SELECT name, title, ability, description FROM ' . tablename('wxapps') . ' WHERE name = :m', array(':m' => $m));
		...
		if(checksubmit('submit')) {
			...
			if(!empty($update)) {
				pdo_update('wxapps', $update, array('name' => $m));
				cache_build_account_modules();
			}
			$sysmodules = system_modules();
			...
			message('更新小程序信息成功', referer(), 'success');
		}
	}
}

if($do == 'installed') {
	$_W['page']['title'] = '已安装的小程序 - 小程序 - 扩展';
	load()->model('module');
	$modtypes = module_types();
	$modules = pdo_fetchall("SELECT * FROM " . tablename('wxapps') .' ORDER BY `issystem` DESC, `mid` ASC', array(), 'mid');
	if (!empty($modules)) {
...
if ($do == 'check') {
	if ($_W['isajax']) {
		...
		if ($foo == 'upgrade') {
			...
			$moduleids = array();
			$modules = pdo_fetchall("SELECT `name` FROM " . tablename('wxapps') . ' ORDER BY `issystem` DESC, `mid` ASC');
			if (!empty($modules)) {
...
if($do == 'prepared') {
	$_W['page']['title'] = '安装小程序 - 小程序 - 扩展';
	$status = $_GPC['status'];
	$recycle_modules = pdo_getall('wxapps_recycle', array(), array(), 'modulename');
...
if($do == 'permission') {
	load()->model('module');
	$id = $_GPC['id'];
	$module = pdo_fetch("SELECT mid, name FROM " . tablename('wxapps') . " WHERE name = :name", array(':name' => $id));
	if(!empty($module[''])) {}
	$isinstall = false;
	$from = '';
		cache_load('wxapps');
	if(!empty($module)) {
		$module = $_W['wxapps'][$module['name']];
	...
	if (empty($module)) {
		message('你访问的小程序不存在. 或许你愿意去系统云服务平台看看. ', ADDONS_URL.'/web/index.php?keyword=' . $_GPC['title']);
	}
...
if($do == 'install') {
	...
	$modulename = $_GPC['m'];
	if (pdo_fetchcolumn("SELECT mid FROM " . tablename('wxapps') . " WHERE name = :name", array(':name' => $modulename))) {
		message('小程序已经安装或是唯一标识已存在！', '', 'error');
	}
	...
	if (!empty($manifest)) {
		$r = cloud_m_prepare($modulename);
		if (is_error($r)) {
			message($r['message'], url('system/wxapp/prepared'), 'error');
		}
	}
	...
	if (empty($manifest)) {
		message('小程序安装配置文件不存在或是格式不正确，请刷新重试！', '', 'error');
	}

	manifest_check($modulename, $manifest);
	$modulepath = IA_ROOT . '/addons/' . $modulename . '/';
	if (!file_exists($modulepath . 'processor.php') && !file_exists($modulepath . 'module.php') && !file_exists($modulepath . 'receiver.php') && !file_exists($modulepath . 'site.php')) {
		message('小程序处理文件 site.php, processor.php, module.php, receiver.php 一个都不存在 ！', '', 'error');
	}
	$module = ext_module_convert($manifest);
	$groups = uni_groups();
	if (!$_W['ispost'] || empty($_GPC['flag'])) {
		template('system/select-groups');
		exit;
	}
	...
	if (pdo_insert('wxapps', $module)) {
		...
		if ($_GPC['flag'] && !empty($post_groups) && $module['name']) {
			foreach ($post_groups as $post_group) {
				...
				$item['wxapps'] = iunserializer($item['wxapps']);
				if (in_array($module['name'], $item['wxapps'])) {
					continue;
				}
				$item['wxapps'][] = $module['name'];
				$item['wxapps'] = iserializer($item['wxapps']);
		...
		if (empty($module_subscribe_success)) {
			message('小程序安装成功, 请按照【公众号服务套餐】【用户组】来分配权限！小程序订阅消息有错误，系统已禁用该小程序的订阅消息，详细信息请查看 <div><a class="btn btn-primary" style="width:80px;" href="' . url('system/subscribe/subscribe') . '">订阅管理</a> &nbsp;&nbsp;<a class="btn btn-default" href="' . url('system/wxapp') . '">返回小程序列表</a></div>', '', 'tips');
		} else {
			message('小程序安装成功, 请按照【公众号服务套餐】【用户组】来分配权限！', url('system/wxapp'), 'success');
		}
	} else {
		message('小程序安装失败, 请联系小程序开发者！');
	}
}

if ($do == 'uninstall') {
	if (empty($_W['isfounder'])) {
		message('您没有卸载小程序的权限', '', 'error');
	}
	$id = $_GPC['id'];

	$module = pdo_fetch("SELECT `name`, `isrulefields`, `issystem`, `version` FROM " . tablename('wxapps') . " WHERE name = :name", array(':name' => $id));

	if (empty($module)) {
		message('小程序已经被卸载或是不存在！', '', 'error');
	}
	if (!empty($module['issystem'])) {
		message('系统小程序不能卸载！', '', 'error');
	}
	if ($module['isrulefields'] && !isset($_GPC['confirm'])) {
		message('卸载小程序时同时删除规则数据吗, 删除规则数据将同时删除相关规则的统计分析数据？<div><a class="btn btn-primary" style="width:80px;" href="' . url('system/wxapp/uninstall', array('id' => $_GPC['id'], 'confirm' => 1)) . '">是</a> &nbsp;&nbsp;<a class="btn btn-default" style="width:80px;" href="' . url('system/wxapp/uninstall', array('id' => $_GPC['id'], 'confirm' => 0)) . '">否</a></div>', '', 'tips');
	} else {
		...
		pdo_insert('wxapps_recycle', array('modulename' => $module['name']));

		message('小程序已放入回收站！', url('system/wxapp'), 'success');
	}
}

if($do == 'upgrade') {
	$id = $_GPC['m'];
	$module = pdo_fetch("SELECT mid, name, version FROM " . tablename('wxapps') . " WHERE name = :name", array(':name' => $id));
	if (empty($module)) {
		message('小程序已经被卸载或是不存在！', '', 'error');
	}
	...
	$module['permissions'] = iserializer($module['permissions']);
	pdo_update('wxapps', $module, array('name' => $id));
	cache_build_account_modules();
	if (!empty($module['subscribes'])) {
		$module_subscribe_success = ext_check_module_subscribe($module['name']);
	}
	if ($_GPC['flag'] == 1) {
		message('小程序更新成功！ <br> 由于数据库更新, 可能会产生多余的字段. 你可以按照需要删除.<div><a class="btn btn-primary" href="' . url('system/database/trim') . '">现在去删除</a>&nbsp;&nbsp;&nbsp;<a class="btn btn-default" href="' . url('system/wxapp/') . '">返回小程序列表</a></div>', '', 'success');
	} else {
		message('小程序更新成功！', referer(), 'success');
	}
}

if($do == 'designer') {
	if(empty($_W['isfounder'])) {
		message('您没有设计新小程序的权限', '', 'error');
	}

	$_W['page']['title'] = '设计新小程序 - 小程序 - 扩展';
	load()->model('module');
	$available = array();
	...
	if(checksubmit() && $available[$_GPC['method']]) {
		...
		if ($_GPC['method'] == 'create') {
			...
			message("生成成功. 请访问 {$mRoot} 继续实现你的小程序.", 'refresh');
			die;
		}
...
function manifest_check($id, $m) {
	if(is_string($m)) {
		message('小程序配置项定义错误, 具体错误内容为: <br />' . $m);
	}
	if(empty($m['application']['name'])) {
		message('小程序名称未定义. ');
	}
	if(empty($m['application']['identifie']) || !preg_match('/^[a-z][a-z\d_]+$/i', $m['application']['identifie'])) {
		message('小程序标识符未定义或格式错误(仅支持字母和数字, 且只能以字母开头). ');
	}
	if(strtolower($id) != strtolower($m['application']['identifie'])) {
		message('小程序名称定义与小程序路径名称定义不匹配. ');
	}
	if(empty($m['application']['version']) || !preg_match('/^[\d\.]+$/i', $m['application']['version'])) {
		message('小程序版本号未定义(仅支持数字和句点). ');
	}
	if(empty($m['application']['ability'])) {
		message('小程序功能简述未定义. ');
	}
	if($m['platform']['isrulefields'] && !in_array('text', $m['platform']['handles'])) {
		message('小程序功能定义错误, 嵌入规则必须要能够处理文本类型消息. ');
	}
	if((!empty($m['cover']) || !empty($m['rule'])) && !$m['platform']['isrulefields']) {
		message('小程序功能定义错误, 存在封面或规则功能入口绑定时, 必须要嵌入规则. ');
	}
	global $points;
	foreach($points as $p => $row) {
		if(is_array($m[$p])) {
...
TPL;
	...
	public function settingsDisplay(\$settings) {
		global \$_W, \$_GPC;
		//点击小程序设置时将调用此方法呈现小程序设置页面，\$settings 为小程序设置参数, 结构为数组。这个参数系统针对不同公众账号独立保存。
...
class {$name}ModuleProcessor extends WeModuleProcessor {
	public function respond() {
		...
		//这里定义此小程序进行消息处理时的具体过程, 请查看微信文档来编写你的代码
	}
}
TPL;
	}
...
class {$name}ModuleReceiver extends WeModuleReceiver {
	public function receive() {
		...
		//这里定义此小程序进行消息订阅时的, 消息到达以后的具体处理过程, 请查看微信文档来编写你的代码
	}
}
TPL;
	}
	...
	if(!$isupdate || empty($module)) {
		...
		if(!empty($module_log)) {
			if(isset($module_log[$module])) {
				pdo_update('wxapps', array('version' => $module_log[$module]), array('name' => $module));
