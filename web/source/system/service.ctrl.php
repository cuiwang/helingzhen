...
if($do == 'post') {
	...
	if(!empty($rid)) {
		...
		if(empty($reply)) {
			message('抱歉，您操作的服务不在存或是已经被删除！', url('system/service', array('m' => $m)), 'error');
		}
	...
	if(checksubmit('submit')) {
		...
		if (!empty($rid)) {
			...
			message('服务保存成功！', url('system/service/post', array('m' => $m, 'rid' => $rid)));
		} else {
			message('服务保存失败, 请联系网站管理员！');
		}
	}
	template('system/service-post');
}
