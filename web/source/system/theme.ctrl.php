...
if($do == 'install') {
	...
	if (!empty($manifest)) {
		...
		if(is_error($r)) {
			message($r['message'], url('system/theme/prepared'), 'error');
		}
	}
...
if($do == 'upgrade') {
	...
	if($batch == 1) {
		...
		if(empty($wait_upgrade)) {
			...
			message('您的模板已经是最新版本', url('system/theme'), 'success');
		}
	...
	if (empty($theme)) {
		if($batch == 1) {
			...
			message($theme['title'] . ' 模板已经被卸载或是不存在。系统将进入下一个模板的更新。<br>请勿关闭浏览器', url('system/theme/upgrade', array('batch' => 1)), 'success');
		}
	...
	if (empty($manifest)) {
		if($batch == 1) {
			cache_write('upgrade:template', iserializer($wait_upgrade));
			message($theme['title'] . ' 模块安装配置文件不存在或是格式不正确。系统将进入下一个模板的更新。<br>请勿关闭浏览器', url('system/theme/upgrade', array('batch' => 1)), 'success');
		}
	...
	if(ver_compare($theme['version'], $packet['version']) != -1) {
		if($batch == 1) {
			cache_write('upgrade:template', iserializer($wait_upgrade));
			message($theme['title'] . ' 模板版本不低于要更新的版本。系统将进入下一个模板的更新。<br>请勿关闭浏览器', url('system/theme/upgrade', array('batch' => 1)), 'success');
		}
	...
	if($batch == 1) {
		cache_write('upgrade:template', iserializer($wait_upgrade));
		message($theme['title'] . ' 模板更新成功。系统将进入下一个模板的更新。<br>请勿关闭浏览器', url('system/theme/upgrade', array('batch' => 1)), 'success');
	}
	message('模板更新成功！', url('system/theme'), 'success');
}
