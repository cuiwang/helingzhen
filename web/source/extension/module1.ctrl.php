...
if ($do == 'bak') {
	...
	if(!is_dir($mdir)){
		itoast('没有可回滚的模块版本，或者上次更新没有修改文件，无需回滚！');
	}
	$time=0;
	$bak = '';
...
if ($do == 'recycle') {
	...
	if ($operate == 'delete') {
		...
		itoast('模块已放入回收站', url('extension/module/prepared', array('status' => 'recycle')), 'success');
	} elseif ($operate == 'recover') {
		pdo_delete('modules_recycle', array('modulename' => $name));
		itoast('模块恢复成功', url('extension/module/install', array('m' => $name)), 'success');
	}
...
if($do == 'info') {
	...
	if($_W['isajax']) {
		...
		if(checksubmit('submit')) {
			...
			itoast('更新模块信息成功', referer(), 'success');
		}
	}
}
...
if($do == 'installed') {
	...
	$modules = pdo_fetchall("SELECT * FROM " . tablename('modules') .' WHERE `wxapp_support` <>2 ORDER BY `issystem` DESC, `mid` ASC', array(), 'mid');
	if (!empty($modules)) {
...
if($do == 'permission') {
	...
	if (empty($module)) {
		itoast('你访问的模块不存在. 或许你愿意去系统云服务平台看看. ', ADDONS_URL.'/web/index.php?keyword=' . $_GPC['title']);
	}
...
if($do == 'install') {
	...
	if (empty($manifest)) {
		itoast('模块安装配置文件不存在或是格式不正确，请刷新重试！', '', 'error');
	}
	manifest_check($modulename, $manifest);
	$modulepath = IA_ROOT . '/addons/' . $modulename . '/';
	if (!file_exists($modulepath . 'processor.php') && !file_exists($modulepath . 'module.php') && !file_exists($modulepath . 'receiver.php') && !file_exists($modulepath . 'site.php')) {
		itoast('模块处理文件 site.php, processor.php, module.php, receiver.php 一个都不存在 ！', '', 'error');
	}
	$module = ext_module_convert($manifest);
	ext_module_clean($modulename);
	$bindings = array_elements(array_keys($points), $module, false);
	...
	if (pdo_insert('modules', $module)) {
		...
		if (empty($module_subscribe_success)) {
			itoast('模块安装成功, 请按照【公众号服务套餐】【用户组】来分配权限！模块订阅消息有错误，系统已禁用该模块的订阅消息，详细信息请查看 <div><a class="btn btn-primary" style="width:80px;" href="' . url('extension/subscribe/subscribe') . '">订阅管理</a> &nbsp;&nbsp;<a class="btn btn-default" href="' . url('extension/module1') . '">返回模块列表</a></div>', '', 'tips');
		} else {
			itoast('模块安装成功, 请按照【公众号服务套餐】【用户组】来分配权限！', url('extension/module1'), 'success');
		}
	} else {
...
if ($do == 'uninstall') {
	...
	if (!empty($module['issystem'])) {
		itoast('系统模块不能卸载！', '', 'error');
	}
	if ($module['isrulefields'] && !isset($_GPC['confirm'])) {
		itoast('卸载模块时同时删除规则数据吗, 删除规则数据将同时删除相关规则的统计分析数据？<div><a class="btn btn-primary" style="width:80px;" href="' . url('extension/module/uninstall', array('id' => $_GPC['id'], 'confirm' => 1)) . '">是</a> &nbsp;&nbsp;<a class="btn btn-default" style="width:80px;" href="' . url('extension/module/uninstall', array('id' => $_GPC['id'], 'confirm' => 0)) . '">否</a></div>', '', 'tips');
	} else {
		...
		itoast('模块已放入回收站！', url('extension/module1'), 'success');
	}
}
...
if($do == 'upgrade') {
	...
	foreach($upfiles as $file) {
		$entry = $dir . $file['path'];
		if(!is_file($entry) || md5_file($entry) != $file['checksum']) {
			itoast("更新失败，请设置好模块根目录（".$dir."）的权限再更新！");
		}
	}
	//模块文件校验
	...
	if(strrchr($manifest['install'],'.')=='.php'){
		itoast('备用更新异常，请移步系统默认更新接口更新该模块！',url('extension/module'));
	}
	...
	//	itoast('模块更新成功！ <br> 系统检测到您的模块目录存在异常文件！请确认是否是自己二次开发所添加的文件.<div><a class="btn btn-primary" href="' . url('system/filecheck/checkmodule') . '">现在查看</a>&nbsp;&nbsp;&nbsp;<a class="btn btn-default" href="' . url('extension/module1/') . '">返回模块列表</a></div>', '', 'success');
	//}
	itoast('模块更新成功！ <br> 由于数据库更新, 可能会产生多余的字段. 你可以按照需要删除.<div><a class="btn btn-primary" href="' . url('system/database/trim') . '">现在去删除</a>&nbsp;&nbsp;&nbsp;<a class="btn btn-default" href="' . url('extension/module1/') . '">返回模块列表</a></div>', '', 'success');
}

if($do == 'designer') {
	if(empty($_W['isfounder'])) {
		itoast('您没有设计新模块的权限', '', 'error');
	}
	...
	if(checksubmit() && $available[$_GPC['method']]) {
		...
		if ($_GPC['method'] == 'create') {
			...
			if(file_exists($mRoot)) {
				itoast("目标位置 {$mRoot} 已存在, 请更换标识或删除现有内容. ");
			}
			mkdirs($mRoot);
			...
			itoast("生成成功. 请访问 {$mRoot} 继续实现你的模块.", 'refresh');
			die;
		}
