...
if($do == 'permission') {
	...
	if (empty($module)) {
		itoast('你访问的模块不存在. 或许你愿意去系统云服务平台看看. ', ADDONS_URL.'http://www.012wz.com' . $_GPC['title']);
	}
...
if($do == 'install') {
	...
	if (pdo_insert('modules', $module)) {
		...
		if (empty($module_subscribe_success)) {
			itoast('模块安装成功, 请按照【公众号服务套餐】【用户组】来分配权限！模块订阅消息有错误，系统已禁用该模块的订阅消息，详细信息请查看 <div><a class="btn btn-primary" style="width:80px;" href="' . url('extension/subscribe/subscribe') . '">订阅管理</a> &nbsp;&nbsp;<a class="btn btn-default" href="' . url('extension/module') . '">返回模块列表</a></div>', '', 'tips');
		} else {
			itoast('模块安装成功, 请按照【公众号服务套餐】【用户组】来分配权限！', url('extension/module'), 'success');
		}
	} else {
...
if ($do == 'uninstall') {
	...
	if ($module['isrulefields'] && !isset($_GPC['confirm'])) {
		...
		itoast('模块已放入回收站！', url('extension/module'), 'success');
	}
}
...
if($do == 'upgrade') {
	...
	if (empty($manifest)) {
		...
		if (is_error($info)) {
			itoast($info['message'], referer(), 'error');
		}
	...
	if ($_GPC['flag'] == 1) {
		itoast('模块更新成功！ <br> 由于数据库更新, 可能会产生多余的字段. 你可以按照需要删除.<div><a class="btn btn-primary" href="' . url('system/database/trim') . '">现在去删除</a>&nbsp;&nbsp;&nbsp;<a class="btn btn-default" href="' . url('extension/module/') . '">返回模块列表</a></div>', '', 'success');
	} else {
		itoast('模块更新成功！', referer(), 'success');
	}
}
