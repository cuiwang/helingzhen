...
if ($do == 'display') {
	...
	if (!empty($parentid)) {
		...
		if (empty($parent)) {
			message('抱歉，上级分类不存在或是已经被删除！', url('pcweb/pccategory/display'), 'error');
		}
	}
	...
	if (checksubmit('submit')) {
		...
		message('更新分类成功！', url('pcweb/pccategory'), 'success');
	}
	template('pcweb/pccategory');
} elseif ($do == 'fetch') {
	...
	if (empty($category)) {
		message('抱歉，分类不存在或是已经被删除！', url('pcweb/pccategory/display'), 'error');
	}
	...
	message('分类删除成功！', url('pcweb/pccategory'), 'success');
} elseif($do == 'check') {
	$styleid = intval($_GPC['styleid']);
