...
if ($do == 'design') {
	...
	if (!empty($_GPC['wapeditor'])) {
		...
		message('页面保存成功.', url('pcweb/pceditor/design', array('id' => $id, 'multiid' => $multiid)), 'success');
	} else {
		$page = pdo_fetch("SELECT * FROM ".tablename('fournet_pcpage')." WHERE id = :id", array(':id' => $id));
		template('pcweb/pceditor');
	}
} elseif ($do == 'page') {
	$_W['page']['title'] = '专题页面 - 微站功能';
	...
	if (!empty($_GPC['wapeditor'])) {
		...
		message('个人中心保存成功.', url('pcweb/pceditor/uc'), 'success');
	}
	...
	if ($_GPC['wapeditor']) {
		...
		message('底部菜单保存成功.', url('pcweb/pceditor/quickmenu', array('multiid' => $multiid, 'type' => $type)), 'success');
	}
	if ($type == '4') {
