...
<div class="clearfix">
	...
	<div class="alert alert-danger">
		1、字母拼音选择无效？: <a href="<?php  echo url('system/uppinyin');?>" style="border-radius: 4px;width: 84%;padding: 4px;font-size: 14px;background-color: #00bc0c;border-color: #00bc0c;color: #FFF">点击更新数据</a> ;
	</div>
			...
			<div class="media">
				<div class="pull-right" style="width:500px;">
					...
					<div class="module-set">
						...
						<span class="label label-primary"> <a style="color:#fff" href="http://www.baidu.com">
						<i class="fa fa-wrench"></i> 百度一下 </a>
						</span>
						...
						<a onclick="return confirm('确定将该模块回滚至上一次更新之前的版本？'); return false;" href="<?php  echo url('extension/module/bak', array('id' => $row['name']))?>">回滚</a>
						<a href="<?php  echo url('system/filecheck/checkmodule', array('name' => $row['name']))?>">文件校验</a>
						<?php  } ?>
						<?php  } ?>
						&nbsp;
			...
			$.post('<?php  echo url('extension/module/check');?>', {foo: 'upgrade'},function(dat){
				try {
					...
					$('.upgrade-label').each(function(){
						...
						if(ret[n]) {
							$('.hide-form[id="' + n + '"]').html('<span class="label label-warning">来自本地安装</span>').show();
							if(ret[n].version > v) {
								var tips = '';
								tips = '来自本地安装';
...
<!---	<div class="alert alert-danger">
		如果无法显示已购买模块，错误原因:
		1、您的服务器屏蔽了云服务器IP；
		2、您的服务器IP地址没有授权，您的服务器IP为:“<?php  echo $ip;?>”（请:<a href="<?php  echo url('cloud/profile');?>" target="_blank">移步注册站点</a>  IP授权管理 提交IP授权）；
