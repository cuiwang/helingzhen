...
function web_url($segment, $params = array()) {
	...
	$url = $_W['siteroot'] . 'web/index.php?c=site&a=entry&m=feng_merchants&';
	if(!empty($do)) {
		$url .= "do={$do}&";
	}
	if(!empty($ac)) {
...
function app_url($segment, $params = array()) {
	global $_W;
	list($do, $ac, $op) = explode('/', $segment);
	$url = $_W['siteroot'] . 'app/index.php?i='.$_W['uniacid'].'&c=entry&m=feng_merchants&';
	if(!empty($do)) {
		$url .= "do={$do}&";
	}
	if(!empty($ac)) {
...
function wl_tpl_ueditor($id, $value = '') {
	...
	if (!defined('TPL_INIT_UEDITOR')) {
		$s .= '<script type="text/javascript" src="../addons/feng_fightgroups/web/resource/components/ueditor/ueditor.config.js"></script><script type="text/javascript" src="../addons/feng_fightgroups/web/resource/components/ueditor/ueditor.all.min.js"></script><script type="text/javascript" src="../addons/feng_fightgroups/web/resource/components/ueditor/lang/zh-cn/zh-cn.js"></script>';
	}
...
/*创建商家账目*/
function createaccount($money,$merchantid){
		global $_W;
		if($merchantid){
...
/*更新商家账目*/
 function updateaccount($money,$merchantid){
		global $_W;
		$f=FALSE;
		if($merchantid){
...
/*模板消息*/
	...
	function merchant(){
		...
		if(empty($merchant)){
			message("未分配帐号，无权查看！您可以去拼团主程序添加商家并分配帐号，用分配的帐号登录多商家后台！");exit;
		}else{
			return $merchant['id'];
		}
	}
