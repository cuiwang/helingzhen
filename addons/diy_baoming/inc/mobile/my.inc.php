...
if ($op=='load') { 		
  ...
  $sql = "SELECT * FROM ".tablename("cgc_baoming_user")." u ,".tablename("cgc_baoming_activity")." a WHERE u.activity_id = a.id  and {$con} ORDER BY u.`id` DESC LIMIT {$start},{$psize}";
  $list = pdo_fetchall($sql);
  
  if(!empty($list)){
		foreach ($list as $key => $value) {
		  $list[$key]['index_logo'] = tomedia($list[$key]['index_logo']);
