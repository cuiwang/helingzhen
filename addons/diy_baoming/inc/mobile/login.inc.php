...
if ($op == "display") {
	...
	//新模板,强制关注,直接在当前页面显示
	if ($activity['must_guanzhu'] && $activity['new_login_mode']){
...
if ($op == "post") {
	...
	if (!empty ($activity['code_type']) && $from_user != $_COOKIE["diy_baoming_parent_" . $uniacid . "_" . $id]) {
	...
	//支付
	if ($baoming_user_id > 0 && ($activity['pay_money'])>0 ) {
		$tid =$baoming_user_id ;
		// 生成订单和支付参数
		$params = array (
			'tid' => $tid, 
