...
function p($name = '')
{
    if ($name != 'perm' && !IN_MOBILE) {
        static $_perm_model;
        if (!$_perm_model) {
            $perm_model_file = YIKE_PLUS_PLUGIN . 'perm/model.php';
    ...
    $model = YIKE_PLUS_PLUGIN . strtolower($name) . '/model.php';
    if (!is_file($model)) {
        return false;
    }
    require $model;
...
function show_jsonp($status = 1, $return = null, $callback = null)
{
    $ret = array(
        'status' => $status
    );
...
{
    global $_W;
    $set  = pdo_fetch('select sets from ' . tablename('yike_plus_sysset') . ' order by id asc limit 1');
...
function shop_template_parse($str, $inmodule = false)
{
    ...
    $str = preg_replace('/{ifp\s+(.+?)}/', '<?php /*yike1908*/ if(cv($1)) { ?>', $str);
    $str = preg_replace('/{ifpp\s+(.+?)}/', '<?php /*yike1908*/ if(cp($1)) { ?>', $str);
    $str = preg_replace('/{ife\s+(\S+)\s+(\S+)}/', '<?php /*yike1908*/ if( ce($1 ,$2) ) { ?>', $str);
    return $str;
}
