...
if($_GPC['op'] == 'sign'){
    ...
    if (date('Y-m-d', $last_sign['sign_time'])==date('Y-m-d', time())) {
        show_jsonp(0,array('result' => '您今日已签到' ,'bet_name' =>$bet_name),$callback);
    }
    ...
    }elseif(date('Y-m-d', time()) > date('Y-m-d', $last_sign['sign_time'] + 24*60*60)){
        $sign_num = 1;
    }
// 当前签到积分
    // 设置基础分数
    ...
    show_jsonp(1,array('result' => '签到成功' ,'bet_name' =>$bet_name),$callback);
}else{
    show_jsonp(1,array('user' => $user ,'bet_name' =>$bet_name),$callback);
}