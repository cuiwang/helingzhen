...
class Core extends WeModuleSite {
    ...
    public function getNaveMenu($schoolid, $action)
    {
        ...
        $do = $_GPC['do'];
        $navemenu = array();
		$school = pdo_fetch("SELECT is_cost,is_recordmac,is_rest,shoucename,is_video,videoname FROM " . tablename($this->table_index) . " WHERE :id = id", array(':id' => $schoolid));
        if ($school['is_video'] == 1 && $school['videoname']) {
			$navemenu[0] = array(
				...
				'items' => array(
					...
					4 => array('title' => '食谱管理', 'url' => $do != 'cook' ? $this->createWebUrl('cook', array('op' => 'display', 'schoolid' => $schoolid)) : '',
			...
			$navemenu[0] = array(
				...
				'items' => array(
					...
					4 => array('title' => '幻灯片管理', 'url' => $do != 'banner' ? $this->createWebUrl('banner', array('op' => 'display', 'schoolid' => $schoolid)) : '',
						'active' => $action == 'banner' ? ' active' : '',
						'append' => array(
							'title' => '<i style="color:#d9534f;" class="fa fa-image"></i>',
						),
		...
		if ($school['is_rest'] == 1 && $school['shoucename']) {
			$navemenu[1] = array(
				...
				'items' => array(
					...
					5 => array('title' => $school['shoucename'], 'url' => $do != 'shoucelist' ? $this->createWebUrl('shoucelist', array('op' => 'display', 'schoolid' => $schoolid)) : '',
	...
	public function sendMobileFzqdshjg($logid) {
		global $_GPC,$_W;
		$log = pdo_fetch("SELECT * FROM " . tablename($this->table_checklog) . " where :id = id", array(':id' => $logid));
		$msgtemplate = pdo_fetch("SELECT bjqshjg FROM ".tablename($this->table_set)." WHERE :weid = weid", array(':weid' => $log['weid']));
		...
		$ttime = date('Y-m-d H:i:s', TIMESTAMP);
		$body = "点击本条消息快速查看 ";
		$openidall = pdo_fetchall("select sid,id,openid from ".tablename($this->table_user)." where sid = '{$log['sid']}'");
		$body  = "点击本条消息查看详情 ";
		$num = count($openidall);
		if ($num > 1){
	...
	public function sendMobileSignshtz($logid) {
		global $_GPC,$_W;
		$log = pdo_fetch("SELECT * FROM " . tablename($this->table_checklog) . " where :id = id", array(':id' => $logid));
		$msgtemplate = pdo_fetch("SELECT * FROM ".tablename($this->table_set)." WHERE :weid = weid", array(':weid' => $log['weid']));
        $template_id = $msgtemplate['bjqshtz'];//消息模板id 微信的模板id		
		$class = pdo_fetch("SELECT tid FROM " . tablename($this->table_classify) . " where sid = :sid And type = 'theclass' ", array(':sid' => $log['bj_id']));
		$teacher = pdo_fetch("SELECT openid,tname FROM " . tablename($this->table_teachers) . " where id = :id And schoolid = :schoolid ", array(':schoolid' => $log['schoolid'],':id' => $class['tid']));
		$student = pdo_fetch("SELECT s_name FROM " . tablename($this->table_students) . " where id = :id ", array(':id' => $log['sid']));
	...
	public function sendMobileBjqshtz($schoolid, $weid, $shername, $bj_id) {
		...
		$teachers = pdo_fetch("SELECT openid FROM " . tablename($this->table_teachers) . " where weid = :weid AND id = :id", array(':weid' => $weid, ':id' => $bzj['tid']));
	...
	public function sendMobileZuoye($notice_id, $schoolid, $weid, $tname, $bj_id) {
		...
		foreach ($userinfo as $key => $value) {
			$openidall = pdo_fetchall("select sid,id,openid from ".tablename($this->table_user)." where sid = '{$value['id']}'");
			$title ="{$km['sname']}老师{$tname}发来一条作业消息!";
			$bjname  = "{$bj['sname']}";
			$body  = "点击本条消息查看详情 ";
	...
	public function sendMobileBjtz($notice_id, $schoolid, $weid, $tname, $bj_id) {
		global $_GPC,$_W;
		$msgtemplate = pdo_fetch("SELECT * FROM ".tablename($this->table_set)." WHERE :weid = weid", array(':weid' => $weid));
		$notice = pdo_fetch("SELECT * FROM ".tablename($this->table_notice)." WHERE :weid = weid AND :id = id AND :schoolid = schoolid", array(':weid' => $weid, ':id' => $notice_id, ':schoolid' => $schoolid));
	...
	public function sendMobileFxsc($weid, $schoolid, $tname, $sid, $scid, $setid) {
		...
		$msgtemplate = pdo_fetch("SELECT bjtz FROM ".tablename($this->table_set)." WHERE :weid = weid", array(':weid' => $weid));
        $template_id = $msgtemplate['bjtz'];//消息模板id 微信的模板id
		...
		foreach ($userinfo as $key => $value) {
			...
			$openid = $openidall['0']['openid'];
			$this->sendtempmsg($template_id, $url, $data, '#FF0000', $value['openid']);
	...
	public function sendMobileXsqjsh($leaveid, $schoolid, $weid, $tname) {
		...
		$touser = pdo_fetch("SELECT id FROM " . tablename($this->table_user) . " where schoolid = :schoolid AND sid = :sid AND uid = :uid AND openid = :openid", array(':schoolid' => $schoolid, ':sid' => $leave['sid'], ':uid' => $leave['uid'], ':openid' => $leave['openid']));
	...
	public function sendMobileLyhf($leave_id, $schoolid, $weid) {
		global $_GPC,$_W;
		...
		$teacher2 = pdo_fetch("SELECT tname FROM " . tablename($this->table_teachers) . " where weid = :weid AND id = :id", array(':weid' => $weid, ':id' => $touser['tid']));
		$template_id = $msgtemplate['liuyan'];//消息模板id 微信的模板id
		mload()->model('user');
		if($user['sid']){
		...
		if (!empty($template_id)) {
			$this->sendtempmsg($template_id, $url, $data, '#FF0000', $touser['openid']);
		}
	}	
	...
	public function sendMobileJsqj($leave_id, $schoolid, $weid, $openid) {
	    ...
	    $stime = date('Y-m-d H:i:s', $leave['startime1']);
	    $etime = date('Y-m-d H:i:s', $leave['endtime1']);
		$time = "{$stime}至{$etime}";
		$body = "点击本条消息快速处理 ";
	    $datas=array(
		'name'=>array('value'=>$_W['account']['name'],'color'=>'#173177'),
	...
	public function sendMobileJxlxtz($schoolid, $weid, $bj_id, $sid, $type, $leixing, $id, $pard) {
		...
		$userinfo = pdo_fetchall("SELECT id,pard FROM ".tablename($this->table_user)." where weid = :weid And schoolid = :schoolid And sid = :sid",array(':weid'=>$weid, ':schoolid'=>$schoolid, ':sid'=>$sid));
	...
	public function sendMobileFzqdtx($schoolid, $weid, $bj_id, $sid, $type, $leixing, $id, $pard) {
		global $_GPC,$_W;
		$msgtemplate = pdo_fetch("SELECT * FROM ".tablename($this->table_set)." WHERE :weid = weid", array(':weid' => $weid));
        $template_id = $msgtemplate['jxlxtx'];//消息模板id 微信的模板id
		$student = pdo_fetch("SELECT s_name FROM " . tablename($this->table_students) . " where id = :id ", array(':id' => $sid));
		$log = pdo_fetch("SELECT * FROM " . tablename($this->table_checklog) . " where id = :id ", array(':id' => $id));
		$openidall = pdo_fetchall("select sid,id,openid,pard from ".tablename($this->table_user)." where sid = :sid ", array(':sid' => $log['sid']));
		$num = count($openidall);
		if ($num > 1){
			foreach ($openidall as $key => $values) {
				if($values['sid']){
					...
					$this->sendtempmsg($template_id, $url, $data, '#FF0000', $values['openid']);		
				}
			}
		}else{
			if($openidall['0']['sid']){
				...
				$this->sendtempmsg($template_id, $url, $data, '#FF0000', $openidall['0']['openid']);
			}	
		}
	}
	
	public function sendMobileJfjgtz($orderid) {
		global $_W;
		$order = pdo_fetch("SELECT * FROM " . tablename($this->table_order) . " where id = :id ", array(':id' => $orderid));
		$msgtemplate = pdo_fetch("SELECT * FROM ".tablename($this->table_set)." WHERE :weid = weid", array(':weid' => $order['weid']));
        $template_id = $msgtemplate['jfjgtz'];//消息模板id 微信的模板id		
		$student = pdo_fetch("SELECT s_name FROM " . tablename($this->table_students) . " where id = :id ", array(':id' => $order['sid']));
		$userinfo = pdo_fetchall("SELECT id,pard,openid FROM ".tablename($this->table_user)." where schoolid = :schoolid And sid = :sid",array(':schoolid'=>$order['schoolid'], ':sid'=>$order['sid']));
		foreach ($userinfo as $key => $value) {
