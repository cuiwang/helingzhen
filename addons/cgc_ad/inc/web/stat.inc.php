<?php 
		...
		//撒钱统计
	  	...
	  	$puser=pdo_fetchcolumn("SELECT count(t.id) FROM  (SELECT id FROM ".tablename('cgc_ad_adv')." WHERE weid=".$weid.$quan_id." and del=0 group by mid ) t");

		$_pnum=pdo_fetchcolumn("SELECT count(id) FROM   ".tablename('cgc_ad_adv')." WHERE weid=".$weid.$quan_id." and del=0");

		$_pcount = pdo_fetchcolumn("SELECT SUM(total_pay) FROM ".tablename('cgc_ad_adv')." WHERE weid=".$weid.$quan_id." and status>0 and del=0");
		...
		//vip充值金额
		$_vip_total = pdo_fetchcolumn("SELECT SUM(pay) FROM ".tablename('cgc_ad_vip_pay')." WHERE weid=".$weid .$quan_id." and pay>0 and status=1"); //vip充值金额
							 ...
							 $sql = "SELECT a.*,c.headimgurl,c.nickname FROM ".tablename('cgc_ad_adv')." a
							  ...
							  LIMIT ".($pindex - 1) * $psize.",{$psize}";
								...
								$sql2 = "SELECT count(id) FROM ".tablename('cgc_ad_adv')." a
								 ...
								 $sql3 = "SELECT SUM(total_pay) FROM ".tablename('cgc_ad_adv')." a
