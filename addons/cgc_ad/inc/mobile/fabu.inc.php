...
$type_list = $this->get_type_list($quan_id,0,0);
...
//手续费
$handling_fee = floatval($quan['handling_fee']);
if($handling_fee>0){
...
if ($op == 'display') {
  $id=$_GPC['id']; 
  if (!empty($id)){
    $adv=pdo_fetch("SELECT * FROM ".tablename('cgc_ad_adv')." where weid=$weid and id=$id and  mid=$mid  and quan_id=$quan_id and del=0 ");
  } else {
    $adv=pdo_fetch("SELECT * FROM ".tablename('cgc_ad_adv')." where weid=$weid  and  mid=$mid  and quan_id=$quan_id and del=0 and model=1 and status=0 order by id desc");
  }  
...
if ($op == 'add') {
	...
	if ($adv_id > 0) {
		...
		if(!empty($_GPC['cmd'])){
			if($_GPC['cmd']=='preview'){
				$redirect=$_W['siteroot'] . 'app/' . substr($this->createMobileUrl('detail',array('cmd'=>'preview','quan_id'=>$quan_id,'id'=>$adv_id,'model'=>1)), 2);
