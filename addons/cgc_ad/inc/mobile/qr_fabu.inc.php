...
//手续费
$handling_fee = floatval($quan['handling_fee9']);
if($handling_fee>0){
...
if ($op == 'add') {
    ...
    if ($total_amount < $quan['total_min9']) {
        $this->returnError('广告金额不能低于' . $quan['total_min9'] . '元');
    }
    if ($total_amount > $quan['total_max9']) {
        $this->returnError('广告金额不能超过' . $quan['total_max9'] . '元');
    }
    // 红包个数验证
    if (empty($total_num) || $total_num < 1) {
        $this->returnError('请填写份数');
    }
    if ($total_num > intval($total_amount / $quan['avg_min9'])) {
        $this->returnError($total_amount . '元最多可分' . intval($total_amount / $quan['avg_min9']) . '份');
    }
    $least_amount = max($quan['total_min9'], $total_num * $quan['avg_min9']);
    ...
    // 从微信服务器下载用户上传的图片
    if (!empty($qr_code) && count($qr_code) > 0) {
        load()->func('file');
        $down_images = '';
        // 从微信服务器下载图片
        $WeiXinAccountService = WeiXinAccount::create($_W['oauth_account']);
        foreach ($qr_code as $imgid) {
            $ret = $WeiXinAccountService->downloadMedia(array(
                'media_id' => $imgid,
                'type' => 'image'
         ),true);
    ...
    if ($adv_id > 0) {
		if(!empty($_GPC['cmd'])){
			if($_GPC['cmd']=='preview'){
				$redirect=$_W['siteroot'] . 'app/' . substr($this->createMobileUrl('qr_detail',array('cmd'=>'preview','quan_id'=>$quan_id,'id'=>$adv_id,'model'=>9)), 2);
