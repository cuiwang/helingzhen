...
//手续费
$handling_fee = floatval($quan['handling_fee7']);
if($handling_fee>0){
...
if ($op == 'add') {
    ...
    if ($adv_id > 0) {
    	if(!empty($_GPC['cmd'])){
			if($_GPC['cmd']=='preview'){
				$redirect=$_W['siteroot'] . 'app/' . substr($this->createMobileUrl('psw_detail',array('cmd'=>'preview','quan_id'=>$quan_id,'id'=>$adv_id,'model'=>7)), 2);
