    ...
    if($op=='display'){
      ...
      if(empty($images)){
		...
		if(!empty($images) && count($images)>0){
      		...
      		//$WeiXinAccountService = WeAccount::create($_W['acid']);  
     	  ...
     	  if (!empty($config['task_template_id']) && !empty($quan['task_submit_switch'])){
            ...
            " WHERE   weid=".$weid." AND quan_id=".$quan_id." and openid='{$adv['openid']}'");
     	    if  ($adv_member['message_notify']) {   	  
	           ...
	           $_url= $_W['siteroot']."app/".substr($this->createMobileUrl('foo',array('form'=>"task_check",'quan_id'=>$quan['id'],'id'=>$id,'task_id'=>$task_id)), 2);	    
