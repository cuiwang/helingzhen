...
$type_list =$this-> get_type_detail($_GPC['parent_id']);
...
$con.= ' and (a.info_type_id='.intval($_GPC['info_type'])." or a.parent_type_id=".intval($_GPC['info_type']).")";

if (!empty($_GPC['child_id']) && $_GPC['child_id']!=$_GPC['info_type']){
$con = ' and a.info_type_id='.intval($_GPC['child_id']);
}
...
if($_GPC['dopost']=='ajax'){
	...
	if(empty($quan['templet_id'])){
		  ...
		  if ($item['title']){
		   ...
		   if ($item['summary']){
			...
			if($item['rob_end_time']>0){
				...
				if ($quan['is_page_addr']){
           	       $item['city1']=empty($item['city1'])?"全国":$item['city1'];
           		   $ht.='<span class="show-citiy_icon"><i>'.$item['city1'].'</i></span>';												
           		}
		...
		foreach ($list as $key => $item) {
			if ($settings['rob_diff']){
				if($item['model']=='8'){//卡券
					$item['is_redbag']=pdo_fetchcolumn("SELECT count(id) FROM ".tablename('cgc_ad_red')." WHERE weid=".$weid." AND quan_id=".$quan_id." AND advid=". $item['id'] ." AND mid=".$mid);
				}
				else{
					$item['is_redbag']=pdo_fetchcolumn("SELECT count(id) FROM ".tablename('cgc_ad_couponc')." WHERE weid=".$weid." AND quan_id=".$quan_id." AND advid=". $item['id'] ." AND mid=".$mid);
				}
			}
	       ...
	       $ht.='<div class="grxx" style="padding-bottom:0px; margin:0;">';
			 ...
			 $ht.=' <div class="grxxl"><span class="xxright dbnr">';
            	 ...
            	 $ht.=' <div class="grxxl"><span class="xxright dbnr">';       
                 $ht.=$item['is_redbag']>0?'<i style=" color:red;">已抢</i>':'';
...
$list=pdo_fetchall("SELECT a.*,substring_index(a.city,',',1) as city1,REPLACE(a.city,',','') as city2  FROM ".tablename('cgc_ad_adv')." as a
...
if ((empty($_GPC['child_id'])  ||  $_GPC['child_id']==$_GPC['info_type']) && !empty($_GPC['parent_id'])){
  $banner=pdo_fetchall("SELECT * FROM ".tablename('cgc_ad_type_banner')." WHERE weid=".$weid." AND type_id=".$_GPC['parent_id']." AND status=1 ORDER BY displayorder DESC,id ASC");
}
//统计人数
...
include $this->template('index2_list');
