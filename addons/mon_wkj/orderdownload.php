...
if (!empty($starttime) && !empty($endtime)) {
	$list= pdo_fetchall("select * from ".tablename(DBUtil::$TABLE_WJK_ORDER)." where kid=:kid and createtime >=:starttime and createtime <=:endtime order by createtime desc ",array(':kid'=>$kid, ':starttime' => $starttime, ':endtime'=> $endtime));
} else {
