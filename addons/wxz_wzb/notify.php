...
if($sign == $get['sign']) {
	...
	if(!is_error($site)) {
		$item = pdo_fetch('SELECT * FROM ' . tablename('wxz_wzb_comment') . ' WHERE `dsid` = :dsid', array(':dsid' => substr($get['out_trade_no'],16)));
		pdo_update('wxz_wzb_ds',array('status'=>'1'),array('id'=>substr($get['out_trade_no'],16)));
		pdo_update('wxz_wzb_comment',array('dsstatus'=>'1'),array('dsid'=>substr($get['out_trade_no'],16)));
		$method = 'payResult';
