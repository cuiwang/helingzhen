...
$fashb = pdo_fetch( " SELECT is_ty FROM ".tablename('haoman_dpm_hb_setting')." WHERE rid='".$rid."' " );

$bpmoney = pdo_fetchall("select * from " . tablename('haoman_dpm_bpmoney') . " where rid = :rid and uniacid=:uniacid and bp_type=1 order by `bp_time` asc", array(':rid' => $rid, ":uniacid" => $uniacid));
$first = $bpmoney[0]['id'];
if($bpmoney){
    $money  =1;
}

if(empty($bpmoney)||$fashb['is_ty']==0){
    message('涂鸦功能暂未开启！');
}
...
include $this->template('mob_bp_sq');
