...
class Feng_fightgroupsModuleSite extends WeModuleSite {
	...
	public function updategourp() {
		global $_W;
		$now = time();
		$allgroups = pdo_fetchall("select *from" . tablename('tg_group') . "where groupstatus=3 and uniacid='{$_W['uniacid']}' and status = 1 ");
		foreach ($allgroups as $key => $value) {
			if ($value['endtime'] < $now && $value['lacknum'] > 0) {
				...
				foreach ($orders as $k => $v) {
					$res = pdo_update('tg_order', array('status' => 6), array('tuan_id' => $value['groupnumber']));
				}
			}
		}
	}
