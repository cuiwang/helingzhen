 ...
 */
	...
	if(!array_key_exists($params['module'], $moduels)) wl_message('模块不存在.');//若模块不在微赞则报错。
	
	$pars = array();
	$pars[':uniacid'] = $order['uniacid'];
	$pars[':module'] = $params['module'];
	$pars[':tid'] = $params['tid'];
	$log = pdo_fetch('SELECT * FROM ' . tablename('core_paylog') . ' WHERE `uniacid`=:uniacid AND `module`=:module AND `tid`=:tid', $pars); //若果微赞支付记录存在则取出。
...
if ($_W['isajax']) {
	...
	if(!is_array($setting['payment'])) wl_message(array('errno'=>1,'message'=>"没有有效的支付方式, 请联系网站管理员.")); //判断微赞是否有支付设置内容。
	...
	if (!empty($setting['payment']['credit']['switch'])) $credtis = mc_credit_fetch($_W['member']['uid']); //如果微赞余额开启了，则获取微赞余额。
	
	$log = pdo_fetch('SELECT * FROM ' . tablename('core_paylog') . ' WHERE `uniacid`=:uniacid AND `module`=:module AND `tid`=:tid', $pars); // 取出微赞支付记录。
	
	if(!empty($log) && $type != 'credit' && $log['status'] != '0') wl_message(array('errno'=>1,'message'=>"这个订单已经支付成功, 不需要重复支付!"));
	$uniontid = $order['orderno'];
	pdo_update('core_paylog', array('type'=>$type,'uniontid'=>$uniontid), array('plid' => $log['plid'])); //更新type和uniontid到微赞core_paylog。
