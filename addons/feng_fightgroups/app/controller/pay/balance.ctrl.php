<?php 
	...
	if(!empty($log) && $log['status'] == '0') {
		...
		$credit_type = $tgsetting['credit_type']?$tgsetting['credit_type']:1; //$credit_type=1为微赞积分余额
		...
		if($credit_type==1){ //微赞积分余额
				if (empty($uid)){
					$setting['payment']['credit']['switch'] = false; //如果uid为空则关闭余额支付
				message("微赞余额未开启");
				} 
			if ($setting['payment']['credit']['switch']) $credtis = mc_credit_fetch($uid);  //若果微赞余额支付开启 获得余额
