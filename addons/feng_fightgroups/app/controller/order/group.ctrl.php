  	...
  	if($tuaninfo['iflottery'] && $tuaninfo['lottery_id']>0) {
		...
		if($lottery['pattern'] == 2){
			switch($myorder['lottery_status']){
		    		...
		    		case 7: $myStatus = ($tuaninfo['groupstatus']==2)?'三等奖':'待抽奖';$myStatus2 = ($tuaninfo['groupstatus']==2)?'恭喜获得三等奖':'';break;
		    ...
		    foreach($ordersLottery as$key =>&$value){
				...
				if($lottery['pattern'] == 2){
					switch($value['lottery_status']){
			    		...
			    		case 7: $ordersLottery[$key]['lottery_status_name'] = ($tuaninfo['groupstatus']==2)?'三等奖':'待抽奖';break;
