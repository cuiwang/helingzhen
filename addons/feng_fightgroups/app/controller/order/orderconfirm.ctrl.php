...
$openid = $_W['openid'];
$uid = $_W['member']['uid'];
$mid = pdo_getcolumn('tg_member',array('uid'=>$uid),'id');
...
if($goods['forcegroup'] && empty($tuan_id) && $groupnum > 1){ //强制成团
	$tuan_id = $_SESSION['tuan_id'] = pdo_getcolumn('tg_group',array('uniacid'=>$_W['uniacid'],'goodsid'=>$id,'groupstatus'=>3),'groupnumber');
}
