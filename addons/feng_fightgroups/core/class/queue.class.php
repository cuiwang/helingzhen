	...
	public function doTask(){
		...
		foreach($message as $k=>$v){
			...
			if($v['key'] == 2){
				...
				//给一等奖发消息
				$defaultFirst = pdo_fetchall("select openid,id,tuan_id from".tablename('tg_order')."where lotteryid={$v['value']} and lottery_status in(2,5)");
				...
				if($lottery['nogetmessage']==1 && $lottery['dostatus']==1){
					$noGetOrders = pdo_fetchall("select openid,id,tuan_id from".tablename('tg_order')."where lotteryid={$v['value']} and lottery_status not in(2,3,4,5,6,7)");//三等奖
			...
			if($v['key'] == 3){ //成团就开奖  通知及更新奖品数据
				...
				//给一等奖发消息
				$defaultFirst = pdo_fetchall("select openid,id,tuan_id from".tablename('tg_order')."where lottery_status in(2,5)  and tuan_id={$order['tuan_id']} and status in(2,6,7) ");
				...
				//二等奖中奖发送优惠券
				$openidsFirst = pdo_fetchall("select openid,id,tuan_id from".tablename('tg_order')."where lottery_status in(3,6) and tuan_id='{$order['tuan_id']}' and status in(2,6,7) "); 
				if ($prize['first']['radio']==1){
					$content = "很遗憾您与大奖擦肩而过,我们已将抽奖金额全额退还给您。";
				}
				if ($prize['first']['radio']==2 || $prize['first']['radio']==3){
				...
				//三等奖中奖发送优惠券
				$url='';
				$openidsSecond = pdo_fetchall("select openid,id,tuan_id from".tablename('tg_order')."where lottery_status in(4,7) and tuan_id='{$order['tuan_id']}' and status  in(2,6,7) ");//三等奖
				if ($prize['second']['radio']==1){
					$content = "很遗憾您与大奖擦肩而过,我们已将抽奖金额全额退还给您。";
				}
				if ($prize['second']['radio']==2 || $prize['second']['radio']==3){
