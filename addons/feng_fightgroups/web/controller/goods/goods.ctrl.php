...
load()->func('tpl');
$ops = array('display', 'post', 'single_op', 'batch','setgoodsproperty','copy','selectgift','taobaourl','asdasd','tag','pass','param');
...
if ($op == 'display') {
	...
	if(checksubmit()){
		...
		for($i=0;$i<count($ids);$i++){
			...
			Util::deleteCache('goods', $ids[$i]);//删除缓存efwww_com
		}
	...
	$where['#g_type#'] = $_GPC['g_type']?"(".$_GPC['g_type'].")":'(0,1,4)';
...
if ($op == 'post') {
	...
	if (!empty($goods))$goodsOption = model_goods::getSingleGoodsOption($id);//商品规格efwww_com
