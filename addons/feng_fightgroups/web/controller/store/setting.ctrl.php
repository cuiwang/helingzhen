...
$goodsops = array('display', 'post', 'single_op', 'batch','setgoodsproperty','copy','taobaourl','pass','param');
$goodsop_names = array('商品列表','新增/修改商品','上下架/售罄/删除/彻底删除','批量设置','设置商品属性','复制商品','导入淘宝信息','评论审核','标签属性');
...
$groupop_names = array('团列表','团详情','后台核销','导出');
	...
	if (checksubmit('submit')) {
		if($_GPC['secretkey']){
			$secretkey = $_GPC['secretkey'];
			 $resp = ihttp_post('../addons/weliam_manage/api/grant.api.php', array(
		        'type' => 'grant',
