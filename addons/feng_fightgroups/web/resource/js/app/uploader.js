define(["jquery", "underscore", "webuploader", "jquery.jplayer"], function(a, b, c) {
	var d = {
		defaultoptions: {
			direct: !1,
			callback: null,
		...
		init: function(b, c) {
			var d = this;
			return d.options = a.extend({}, d.defaultoptions, c), d.options.callback = b, a("#modal-webuploader").remove(), 0 == a("#modal-webuploader").length && a(document.body).append(d.buildHtml().mainDialog), d.modalobj = a("#modal-webuploader"), d.modalobj.modal("show"), d.modalobj.on("shown.bs.modal", function() {
		...
		initRemote: function() {
			...
			b.modalobj.find("#li_network").removeClass("hide"), b.modalobj.find(".modal-body").append(b.buildHtml().remoteDialog), b.modalobj.find(".btn-primary").click(function() {
				...
				c.length > 0 && a.getJSON("./index.php?c=utility&a=file&do=fetch", {
					...
					a.errno || (console.log(a), b.finish([a.attachment]))
				})
			})
		},
		initLocal: function() {
		...
		localPage: function(c) {
			var d = this,
				e = d.modalobj.find("#history_image");
			return a.getJSON("./index.php?c=utility&a=file&do=local", {
				page: c
			}, function(c) {
				e.find(".history-content").html('<i class="fa fa-spinner fa-pulse"></i>'), b.isEmpty(c.items) || (e.data("attachment", c.items), e.find(".history-content").empty(), e.find(".history-content").html(b.template(d.buildHtml().localDialogLi)(c)), e.find("#image-list-pager").html(c.page), e.find(".pagination a").click(function() {
					d.localPage(a(this).attr("page"))
				}), e.find(".img-list li").click(function(b) {
					d.selectImage(a(b.target).parents("li"))
				}))
			}), e.find(".btn-primary").unbind("click").click(function() {
				var b = [];
				e.find(".img-item-selected").each(function() {
		...
		selectImage: function(b) {
			var c = this;
			a(b).toggleClass("img-item-selected"), c.options.direct && c.modalobj.find("#history_image").find(".btn-primary").trigger("click")
		},
		...
		localAudioPage: function(c) {
			...
			return a.getJSON("./index.php?c=utility&a=file&do=local&type=audio&pagesize=5", {
				...
				e.find(".history-content").html('<i class="fa fa-spinner fa-pulse"></i>'), b.isEmpty(c.items) ? e.find(".history-content").html("") : (e.data("attachment", c.items), e.find(".history-content").empty(), e.find(".history-content").html(b.template(d.buildHtml().localAudioDialogLi)(c)), e.find("#image-list-pager").html(c.page), e.find(".pagination a").click(function() {
					d.localAudioPage(a(this).attr("page"))
				}), e.find(".js-btn-select").click(function(b) {
					a(b.target).toggleClass("btn-primary"), d.options.direct && d.modalobj.find("#history_audio").find(".modal-footer .btn-primary").trigger("click")
				}), d.playAudio())
		...
		initImageUploader: function() {
			...
			q = a.extend({}, q, i.options.uploader), q.pick.multiple = i.options.multiple, d = c.create(q), d.uploadedFiles = [], d.addButton({
				...
				return c.errno ? (alert(c.message), void d.resetUploader()) : void(c.attachment && (c = c.attachment, accept++, d.uploadedFiles.push(c), a("#" + b.id).append('<span class="success" style="line-height: 50px;">' + c.width + "x" + c.height + "</span>"), a(".accept").text("成功上传 " + accept + " 张图片")))
		...
		initAudioUploader: function() {
				...
				if (a !== state) {
					switch (n.removeClass("state-" + state), n.addClass("state-" + a), state = a, state) {
						...
						case "confirm":
							...
							break;
						case "finish":
							if (n.removeClass("disabled"), b = d.getStats(), console.dir(d.uploadedFiles), b.successNum) {
								if (d.uploadedFiles.length > 0) return i.finish(d.uploadedFiles), void d.resetUploader()
							} else state = "done", location.reload()
					}
					g()
			...
			q = a.extend({}, q, i.options.uploader), a("#dndArea p").html("最大支持 " + c.formatSize(q.fileSizeLimit) + " MB 以内的语音 (" + q.accept.extensions + " 格式)"), q.pick.multiple = i.options.multiple, d = c.create(q), d.uploadedFiles = [], d.addButton({
				...
				fileCount = 0, fileSize = 0, accept = 0, d.uploadedFiles = [], a.each(d.getFiles(), function(a, b) {
					e(b)
				}), f(), d.reset(), d.refresh(), a("#dndArea").removeClass("element-invisible"), a("#uploader").find(".filelist").empty(), a("#filePicker").find(".webuploader-pick").next().css({
					left: "190px"
				});
				...
				return c.errno ? (alert(c.message), void d.resetUploader()) : void(c.attachment && (c = c.attachment, accept++, d.uploadedFiles.push(c), a("#" + b.id).append('<span class="success" style="line-height: 50px;">' + c.width + "x" + c.height + "</span>"), a(".accept").text("成功上传 " + accept + " 个音频")))
			}), d.onError = function(a) {
				return "Q_EXCEED_SIZE_LIMIT" == a ? void alert("错误信息: 音频大于 " + c.formatSize(q.fileSizeLimit) + " 无法上传.") : "F_DUPLICATE" == a ? void alert("错误信息: 不能重复上传音频.") : "Q_EXCEED_NUM_LIMIT" == a ? void alert("错误信息: 上传图片数量过多.") : void alert("Eroor: " + a)
			}, n.on("click", function() {
				return a(this).hasClass("disabled") ? !1 : void("ready" === state ? d.upload() : "paused" === state ? d.upload() : "uploading" === state && d.stop())
			}), m.on("click", ".retry", function() {
				d.retry()
