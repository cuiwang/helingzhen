<?php 
	...
	if($op == 'display'){
		...
		include $this->template('member/myshare');
	}
	if($op == 'allshare'){
		$condition_share = !empty($_GPC['goodsid']) ? ' and goodsid='+$_GPC['goodsid'] : ' ';
		$re_share = pdo_fetchall("select * from".tablename('weliam_indiana_showprize')." where uniacid=:uniacid and type=:type and openid=:openid {$condition_share} order by createtime desc limit ".$pagestart.",".$pagenum,array(':uniacid'=>$_W['uniacid'],':type'=>0,':openid'=>$openid));
	...
	if($op == 'speech'){
		//言论检索
		$re_speech = pdo_fetchall("select * from".tablename('weliam_indiana_showprize')." where uniacid=:uniacid and type=:type  and openid=:openid order by createtime desc limit ".$pagestart.",".$pagenum,array(':uniacid'=>$_W['uniacid'],':type'=>1,':openid'=>$openid));
