<?php
	...
	if ($operation == 'list') {
		...
		if(!empty($period[4]['id'])){
			...
			include $this->template('order/order_get');
			exit;
		}else{
		...
		exit;
	}else if($operation == 'alone_detail'){
		$orderid = $_GPC['orderid'];
		$order = pdo_get('weliam_indiana_aloneorder',array('id' => $orderid));
		$goods = pdo_get('weliam_indiana_goodslist',array('id' => $order['goodsid']));
	//	wl_debug($order);
		include $this->template('order/alone_order_get');
		exit;
	}else if ($operation == 'alone_confirmm') {
		$orderid = $_GPC['orderid'];
		$data = array(
			'status'=> 3,
			'taketime'=> TIMESTAMP
		);
		if(pdo_update('weliam_indiana_aloneorder', $data, array('id'=>$orderid))){
			message('确认收货成功！', $this -> createMobileUrl('order_get', array('op' => 'alone_detail','orderid' => $orderid)), 'success');
		}else{
			message('确认收货失败！', $this -> createMobileUrl('order_get', array('op' => 'alone_detail','orderid' => $orderid)), 'error');
		}
		exit;
	}
?>