<?php
		...
		if ($op == 'display') {
			...
			if(!empty($uid)){
				...
				if(empty($goods)){
					message('未找到指定的商户.', $this->createWebUrl('operator'));
				}
			}
			if (checksubmit()) {
				...
				if (!empty($ret)) {
					message('商户信息保存成功', $this->createWebUrl('operator', array('op'=>'add', 'uid'=>$uid)), 'success');
				} else {
					message('商户信息保存失败');
				}
			}
			...
			if(intval($result) == 1){
				message('删除商户成功.', $this->createWebUrl('operator'), 'success');
			} else {
				message('删除商户失败.');
			}
		}
