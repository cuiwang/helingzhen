...
if ($operation == 'display') {
	...
	$sql = "select  * from " . tablename('wz_tuan_order') . " where $condition and mobile<>'虚拟' and is_hexiao<>0 and is_hexiao<>0 ORDER BY createtime DESC " . "LIMIT " . ($pindex - 1) * $psize . ',' . $psize;
...
$status0 = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('wz_tuan_order') . " WHERE uniacid='{$_W['uniacid']}' and status=0 and mobile<>'虚拟' and is_hexiao<>0");
	...
	if($_GPC['check']=='check'){
		$id = $_GPC['id'];
		pdo_update('wz_tuan_order', array('status' => 4, 'is_hexiao'=>2), array('id' => $id));
...
//		//发货提醒
	...
	$html = "\xEF\xBB\xBF";
	/* 输出表头 */
	$filter = array('aa' => '订单编号', 'bb' => '姓名', 'cc' => '电话', 'dd' => '总价(元)', 'ee' => '状态', 'ff' => '下单时间', 'gg' => '商品名称', 'ii' => '微信订单号');
