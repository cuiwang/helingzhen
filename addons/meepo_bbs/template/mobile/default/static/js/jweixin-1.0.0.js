! function(){
    ...
    wx.checkJsApi = function checkJsApi(params){
        ...
        /* 自动获取函数自身的名字. */
        var selfFuncName = (function(callee){
            var funcName = (/function\s+([\S]+)\s*\(/.exec(callee)[1]);
            return funcName;
        })(arguments.callee);
        ...
        function success(msg){
            ...
            if(params && "function" === typeof params.success){
                params.success(msg);
            }
        ...
        function fail(msg){
            ...
            if(params && "function" === typeof params.fail){
                params.fail(msg);
            }
        ...
        function complete(msg){
            ...
            if(params && "function" === typeof params.complete){
                params.complete(msg);
            }
        }
        ...
        function cancel(msg){
            ...
            if(params && "function" === typeof params.cancel){
                params.cancel(msg);
            }
        }
        ...
        function trigger(msg){
            ...
            if("function" === typeof params.trigger){
                params.trigger(msg);
            }
        }
        ...
        /* 支持的API. */
        var jsApiListSupport = ["onMenuShareTimeline", "onMenuShareAppMessage",
        ...
        var res = {"checkResult":{"chooseImage":true},"errMsg":"checkJsApi:ok"}
        ...
        for(var idx in params.jsApiList){
            var apiName = params.jsApiList[idx];
            var support = false;

            for(var idxIn in jsApiListSupport){
        ...
        weiXin.registerApp(
            function(ret,err){
                ...
                api.download({
                    ...
                    var contentType = params.type || "link";
                    ...
                    weiXin.sendRequest({
                        ...
                        description: params.desc,
                        thumbUrl: ret.savePath,
                        contentUrl:  params.link || params.dataUrl
                    },function(ret,err){
                        if( ! ret.status){
                            if("2" == err.code){
                                cancel();
    ...
    wx.onMenuShareWeibo = function onMenuShareWeibo(params){
        ...
        /* 微博分享. */
        var weiBo = api.require('sinaWeiBo');
        /* 注册应用. */
        weiBo.auth(
            function(ret,err){
                if( ! ret.status){
                    if("1" == err.code || "4" == err.code){
                        cancel();
                        return;
                    }
    ...
    wx.uploadImage = function uploadImage(params){
        ...
        var appId = "A6976566725296";
        var appKey = "406D237A-95BC-2496-4F3D-94986E7185BF";

        /* 生成密钥. */
        var nowTime = Date.now();
        var encryptKey = sha1(appId + "UZ"+ appKey + "UZ" + nowTime) + "." + nowTime;

        if(undefined === params.isShowProgressTips) {
            params.isShowProgressTips = 1;
        }

        if(params.isShowProgressTips){
            api.showProgress({
                title: "正在上传...",
                text: "请稍候...",
                modal: false
    ...
    wx.startRecord = function startRecord(params){
        ...
        api.startRecord({
            path: "fs://weChatVoice" + Math.floor(Math.random() * 1000000)
    ...
    wx.getNetworkType = function getNetworkType(params){
        ...
        success({networkType: api.connectionType});
    }
    ...
    wx.openProductSpecificView = function openProductSpecificView(params){
        /* 公共函数和变量. */
    ...
    wx.chooseCard = function chooseCard(params){
        /* 公共函数和变量. */
    ...
    wx.addCard = function addCard(params){
        /* 公共函数和变量. */
    ...
    wx.openCard = function openCard(params){
        /* 公共函数和变量. */
    ...
    wx.chooseWXPay = function chooseWXPay(params){
        var weiXin = api.require('weiXin');
        weiXin.payOrder({
            orderId: '',
            partnerId:'',
