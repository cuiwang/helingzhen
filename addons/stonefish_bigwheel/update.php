...
CREATE TABLE IF NOT EXISTS `ims_stonefish_virtual` (
  `id` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `rid` int(10) unsigned NOT NULL DEFAULT '0' COMMENT '规则id',
  `uniacid` int(10) unsigned NOT NULL DEFAULT '0' COMMENT '公众号ID',
  ...
  `viewnum` int(10) unsigned NOT NULL DEFAULT '0' COMMENT '查看次数',
  `status` tinyint(1) unsigned NOT NULL DEFAULT '1' COMMENT '是否禁止',
  `public`  varchar(255) DEFAULT '' COMMENT '模板缩略图',
  PRIMARY KEY (`id`),
  KEY `indx_rid` (`rid`),
  KEY `indx_uniacid` (`uniacid`)
) ENGINE = MYISAM DEFAULT CHARSET = utf8;
...
if(!pdo_fieldexists('stonefish_bigwheel_reply', 'msgadpic')) {
	pdo_query("ALTER TABLE ".tablename('stonefish_bigwheel_reply')." ADD `msgadpic` varchar(1000) DEFAULT '' COMMENT '消息提示广告图' AFTER `notprizetext`;");
}
if(!pdo_fieldexists('stonefish_bigwheel_reply', 'msgadpictime')) {
	pdo_query("ALTER TABLE ".tablename('stonefish_bigwheel_reply')." ADD `msgadpictime` tinyint(1) unsigned NOT NULL DEFAULT '5' COMMENT '消息提示时效' AFTER `msgadpic`;");
}
if(!pdo_fieldexists('stonefish_bigwheel_reply', 'othermodule')) {
	pdo_query("ALTER TABLE ".tablename('stonefish_bigwheel_reply')." ADD `othermodule` varchar(50) DEFAULT '' COMMENT '其他模块' AFTER `opportunity_txt`;");
}
if(pdo_fieldexists('stonefish_bigwheel_fans', 'inpoint')) {
	pdo_query("ALTER TABLE ".tablename('stonefish_bigwheel_fans')." CHANGE  `inpoint`  `inpoint` float(10,2) unsigned NOT NULL DEFAULT '0.00' COMMENT '起始数';");
}
if(pdo_fieldexists('stonefish_bigwheel_fans', 'outpoint')) {
	pdo_query("ALTER TABLE ".tablename('stonefish_bigwheel_fans')." CHANGE  `outpoint`   `outpoint` float(10,2) unsigned NOT NULL DEFAULT '0.00' COMMENT '已兑换数';");
}
if(pdo_fieldexists('stonefish_bigwheel_fans', 'sharepoint')) {
	pdo_query("ALTER TABLE ".tablename('stonefish_bigwheel_fans')." CHANGE   `sharepoint`  `sharepoint` float(10,2) unsigned NOT NULL DEFAULT '0.00' COMMENT '分享助力';");
}
if(pdo_fieldexists('stonefish_bigwheel_fans', 'sharetime')) {
	pdo_query("ALTER TABLE ".tablename('stonefish_bigwheel_fans')." CHANGE     `sharetime`  `sharetime` int(10) unsigned NOT NULL COMMENT '最后分享时间';");
}
if(pdo_fieldexists('stonefish_bigwheel_fans', 'createtime')) {
	pdo_query("ALTER TABLE ".tablename('stonefish_bigwheel_fans')." CHANGE   `createtime`  `createtime` int(10) unsigned NOT NULL COMMENT '注册时间';");
}
if(pdo_fieldexists('stonefish_bigwheel_fans', 'lasttime')) {
	pdo_query("ALTER TABLE ".tablename('stonefish_bigwheel_fans')." CHANGE    `lasttime`  `lasttime` int(10) unsigned NOT NULL COMMENT '最后参与时间';");
}
