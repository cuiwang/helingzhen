...
if ($op == 'display') {
    ...
    if ($card['status'] == 1 && empty($_W['wlmember']['ncnumber'])) {
        checkgz($qrcode, 1);
        header("Location: " . app_url('member/mycar/display', array(
            'ncnum' => $card['cardsn'],
            'salt' => $card['salt']
        )));
        exit;
    ...
    if ($card['status'] == 2) {
        ...
        if ($carmember['id'] == $_W['mid']) {
            ...
            if ($_W['wlsetting']['merchant']['sendstatus'] == 2 || $_W['wlsetting']['merchant']['rechangestatus'] == 2) {
                ...
                if ($mis) {
                    $sendmsg_url = app_url('app/distance', array(
                        'userid' => $carmember['id']
                    ));
                    if ($_W['wlsetting']['merchant']['sendstatus'] == 2)
                        $movecar_url = app_url('home/movecar', array(
                            'ncnum' => $card['cardsn'],
                            'salt' => $card['salt']
                        ));
                    if ($_W['wlsetting']['merchant']['rechangestatus'] == 2) {
                        $recharge_url = app_url('app/distance', array(
                            'userid' => $carmember['id'],
                            'op' => 'recharge'
                        ));
                        $consume_url  = app_url('app/distance', array(
                            'userid' => $carmember['id'],
                            'op' => 'consume'
                        ));
                    }
            ...
            header("Location: " . app_url('home/movecar', array(
                'ncnum' => $card['cardsn'],
                'salt' => $card['salt']
            )));
            exit;
