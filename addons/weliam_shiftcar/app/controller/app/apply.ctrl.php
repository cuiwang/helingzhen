...
if ($op == 'post') {
    if ($_W['ispost']) {
        ...
        if (empty($_GPC['id'])) {
            ...
            $re = pdo_update('weliam_shiftcar_apply', $data, array(
                'id' => intval($_GPC['id'])
            ));
        }
        if ($re) {
...
if ($op == 'list') {
    $order = pdo_getall('weliam_shiftcar_apply', array(
        'uniacid' => $_W['uniacid'],
        'mid' => $_W['mid']
    ));
...
if ($op == 'get') {
    ...
    if ($type == 1) {
        if ($id) {
            $re = pdo_update('weliam_shiftcar_apply', array(
                'status' => 4
            ), array(
                'id' => $id
            ));
...
if ($op == 'pay') {
    ...
    $order   = pdo_get('weliam_shiftcar_apply', array(
        'uniacid' => $_W['uniacid'],
        'mid' => $_W['mid'],
        'ordersn' => $orderno
    ));
...
if ($op == 'storelist') {
    ...
    foreach ($remark_arr as $key => $value) {
        if (!empty($value['sid'])) {
            $st             = pdo_get('wlmerchant_merchantdata', array(
                'uniacid' => $_W['uniacid'],
                'id' => $value['sid']
            ));
...
if ($op == 'send_mail') {
    if ($_W['ispost']) {
        ...
        $qrcode = pdo_get('weliam_shiftcar_qrcode', array(
            ...
            'sid' => -1,
            'status' => 1,
            'remark' => $_W['wlsetting']['apply']['remark']
        ));
        if ($qrcode) {
    ...
    if ($_W['wlmember']['ncnumber']) {
        ...
        if ($qrcode['model'] == 2) {
            ...
            $showurl = app_url('qr/qrcode/show', array(
                'url' => urlencode($ticket)
            ));
        } else {
            $showurl = app_url('qr/qrcode/show', array(
                'ncnumber' => $_W['wlmember']['ncnumber'],
                'salt' => $qrcode['salt']
            ));
        }
