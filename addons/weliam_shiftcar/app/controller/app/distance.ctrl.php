...
if ($op == 'post') {
    ...
    if (empty($userid) || empty($sid) || empty($sendcontent)) {
        die(json_encode(array(
            "result" => 2,
            'msg' => '缺少重要参数，请重新扫描车主二维码'
        )));
    }
    
    //判断发送次数是否超过限制
    $openid = pdo_get('weliam_shiftcar_member', array(
        'id' => $userid
    ), array(
        'openid',
        'mobile'
    ...
    pdo_insert('wlmerchant_store_notice', array(
        'uniacid' => $_W['uniacid'],
        'mid' => $mid,
        'sid' => $sid,
        'createtime' => time(),
...
if ($op == 'recharge') {
    ...
    if (empty($mis))
        wl_message('抱歉你没有给车主充值的权限', 'close');
    $storename = pdo_getcolumn('wlmerchant_merchantdata', array(
        'uniacid' => $_W['uniacid'],
        'id' => $mis
    ), 'storename');
...
if ($op == 'consume') {
    ...
    if (empty($mis))
        wl_message('抱歉你没有给车主充值的权限', 'close');
    $mccard = pdo_get('weliam_shiftcar_membercard', array(
        'uniacid' => $_W['uniacid'],
        'mid' => $userid,
        'sid' => $mis
    ));
    if (empty($mccard))
        wl_message('抱歉车主还没有本店会员卡，无法核销', 'close');
    $carmember = pdo_get('weliam_shiftcar_member', array(
        'id' => $userid
    ), array(
        'openid',
...
if ($op == 'cardlist') {
    ...
    $cardlist  = pdo_getall('weliam_shiftcar_membercard', array(
        'uniacid' => $_W['uniacid'],
        'mid' => $_W['mid']
    ));
    foreach ($cardlist as $key => &$value) {
        $store              = pdo_get('wlmerchant_merchantdata', array(
            'uniacid' => $_W['uniacid'],
            'id' => $value['sid']
        ), array(
            'storename',
...
if ($op == 're_record') {
    ...
    foreach ($records as $key => &$value) {
        ...
        $value['nickname']   = pdo_getcolumn('wlmerchant_member', array(
            'uniacid' => $_W['uniacid'],
            'id' => $value['remid']
        ), 'nickname');
        $carmember           = pdo_get('weliam_shiftcar_member', array(
...
if ($op == 'memberlist') {
    ...
    foreach ($memberlist as $key => &$value) {
        $value['carmember'] = pdo_get('weliam_shiftcar_member', array(
            'id' => $value['mid']
        ), array(
            'nickname',
            'avatar',
