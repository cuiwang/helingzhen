...
if(WL_EDITION == 'flagship'){
	if ($_W['wlsetting']['pecc']['jisu_api']){
		if ($_W['wlsetting']['pecc']['jisu_appkey']){
			$carmess = model_pecc::get_allcarorg();
			$seek = $carmess['data'];
		}else{
			message('极速数据APPKEY为空(操作：应用-违章提醒-违章设置-接口设置)', $this->createMobileUrl('home'), 'error');
		}
...
if ($op == 'post') {
    ...
    if ($_GPC['ncnumber']) {
        $qrcard = pdo_get('weliam_shiftcar_qrcode', array(
            'cardsn' => trim($_GPC['ncnumber']),
            'uniacid' => $_W['uniacid']
        ));
        if ($qrcard['status'] != 1) {
        ...
        if (pdo_tableexists('wlmerchant_member')) {
            ...
            if ($_W['wlsetting']['merchant']['vipstatus'] == 2 && !empty($_W['wlsetting']['merchant']['viptime'])) {
                $member_s = pdo_get('wlmerchant_member', array(
                    'uniacid' => $_W['uniacid'],
                    'openid' => $_W['openid']
                ), array(
                    'vipstatus',
            ...
            if (!empty($qrcard['sid'])) {
                ...
                if (empty($mid))
                    wl_merge_member($data);
                pdo_insert('wlmerchant_storefans', array(
                    'uniacid' => $_W['uniacid'],
                    'mid' => $mid,
                    'sid' => $qrcard['sid']
                ));
    ...
    $re = pdo_update('weliam_shiftcar_member', $data, array(
        'id' => $_W['mid'],
        'uniacid' => $_W['uniacid']
    ));
    if ($re) {
        if ($_GPC['ncnumber']) {
            if ($_GPC['line'] == 'mail') {
                die(json_encode(array(
                    "result" => 3,
                    'url' => app_url('app/apply/send_mail')
                )));
            }
            die(json_encode(array(
                "result" => 3
