...
if ($op == 'post') {
    if ($_W['mid']) {
        ...
        $re = pdo_update('weliam_shiftcar_member', array(
            'hidestatus' => $status,
            'hidetime' => TIMESTAMP,
            'hidelng' => $_GPC['longitude'],
            'hidelat' => $_GPC['latitude']
        ), array(
            'id' => $_W['mid']
        ));
        if ($re) {
...
if ($op == 'send') {
    ...
    $record      = pdo_getcolumn('weliam_shiftcar_hidenotice', array(
        'mid' => $_W['mid'],
        'uniacid' => $_W['uniacid'],
        'createtime >' => TIMESTAMP - 30 * 60
    ), 'id');
    if ($record) {
        die(json_encode(array(
            "result" => 2,
    ...
    $info = pdo_fetchall('select * from ' . tablename('weliam_shiftcar_member') . ' where uniacid=:uniacid and hidestatus=:hidestatus', array(
        ':uniacid' => $_W['uniacid'],
        ':hidestatus' => 1
    ));
