...
if ($op == 'display') {
    if ($_W['ispost']) {
        ...
        foreach ($list as $key => $value) {
            ...
            $list[$key]['url']        = app_url('member/record/detail', array(
                'id' => $value['id']
            ));
...
if ($op == 'detail') {
    $record      = pdo_get('weliam_shiftcar_record', array(
        'id' => intval($_GPC['id'])
    ));
...
if ($op == 'comment') {
    ...
    pdo_insert('weliam_shiftcar_comment', array(
        'uniacid' => $_W['uniacid'],
        'mid' => $_W['mid'],
        'reid' => $_GPC['reid'],
        'status' => 1,
    ...
    pdo_update('weliam_shiftcar_record', array(
        'comment' => 1
    ), array(
        'id' => $_GPC['reid']
    ));
