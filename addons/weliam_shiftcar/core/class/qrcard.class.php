...
class qrcard
{
    ...
    static function handle($message, $covertype = 'processor')
    {
        ...
        if (strtolower($message['msgtype']) == 'event' && $covertype == 'processor') {
            ...
            if ($card['status'] == 1 && empty($member['ncnumber'])) {
                $returnmess[] = array(
                    'title' => urlencode("您好，绑定挪车卡，点击开始绑定！"),
                    'description' => '',
                    'picurl' => tomedia($settings['bdtuwen']),
                    'url' => app_url('member/mycar/display', array(
                        'ncnum' => $card['cardsn'],
                        'salt' => $card['salt']
                    ))
                );
            ...
            if ($card['status'] == 1 && !empty($member['ncnumber'])) {
                $returnmess[] = array(
                    'title' => urlencode("您好，您确定绑定新的挪车卡吗？"),
                    'description' => urlencode('旧挪车卡将失效，点击开始绑定。'),
                    'picurl' => tomedia($settings['bdtuwen']),
                    'url' => app_url('member/mycar/display', array(
                        'ncnum' => $card['cardsn'],
                        'salt' => $card['salt']
            ...
            if ($card['status'] == 2) {
                ...
                if ($carmember['message']) {
                    $returnmess[] = array(
                        'title' => urlencode("您好，我是【" . $carmember['plate1'] . $carmember['plate2'] . $carmember['plate_number'] . "】车主："),
                        'description' => urlencode($carmember['message'] . "点击呼叫我挪车~~"),
                        'picurl' => tomedia($settings['nctuwen']),
                        'url' => app_url('home/movecar', array(
                            'ncnum' => $card['cardsn'],
                            'salt' => $card['salt']
                        ))
                    );
                    ...
                    $returnmess[] = array(
                        'title' => urlencode("您好，我是【" . $carmember['plate1'] . $carmember['plate2'] . $carmember['plate_number'] . "】车主："),
                        'description' => urlencode('临时停车，给您带来不便尽请谅解！点击呼叫我挪车~~'),
                        'picurl' => tomedia($settings['nctuwen']),
                        'url' => app_url('home/movecar', array(
                            'ncnum' => $card['cardsn'],
                            'salt' => $card['salt']
                ...
                if ($mer_set['rechangestatus'] == 2) {
                    ...
                    if (!empty($retunarr['recharge']))
                        $returnmess[] = $retunarr['recharge'];
                    if (!empty($retunarr['consume']))
                        $returnmess[] = $retunarr['consume'];
                }
                self::send_news($returnmess, $message);
            }
    ...
    static function get_qrid($message)
    {
        ...
        if (!empty($message['ticket'])) {
            if (is_numeric($message['scene'])) {
                $qrid = pdo_fetchcolumn('select id from ' . tablename('qrcode') . ' where uniacid=:uniacid and qrcid=:qrcid', array(
                    ':uniacid' => $_W['uniacid'],
                    ':qrcid' => $message['scene']
                ));
            } else {
                $qrid = pdo_fetchcolumn('select id from ' . tablename('qrcode') . ' where uniacid=:uniacid and scene_str=:scene_str', array(
                    ':uniacid' => $_W['uniacid'],
                    ':scene_str' => $message['scene']
                ));
            }
    ...
    {
        global $_W;
        $card      = pdo_get('weliam_shiftcar_qrcode', array(
            'uniacid' => $_W['uniacid'],
            'qrid' => $qrid
        ), array(
            'sid',
        ...
        $carmember = pdo_get('weliam_shiftcar_member', array(
            'uniacid' => $_W['uniacid'],
            'id' => $card['mid']
        ), array(
            'id',
        ...
        $member    = pdo_get('weliam_shiftcar_member', array(
            'uniacid' => $_W['uniacid'],
            'openid' => $message['from']
        ), array(
            'ncnumber',
    ...
    static function check_qrcode($cardsn, $salt)
    {
        ...
        $qrcode = pdo_get('weliam_shiftcar_qrcode', array(
            'cardsn' => $cardsn,
            'uniacid' => $_W['uniacid']
        ));
    ...
    static function send_storemsg($openid, $user, $qrcode)
    {
        ...
        if (empty($openid) || empty($user))
            return;
        $mid = pdo_getcolumn('wlmerchant_member', array(
            'uniacid' => $_W['uniacid'],
            'openid' => $openid
        ), 'id');
        if (empty($mid) || empty($user))
            return;
        $mis = pdo_getcolumn('wlmerchant_merchantuser', array(
            'uniacid' => $_W['uniacid'],
            'mid' => $mid,
            'status' => 2,
            'enabled' => 1
        ...
        if ($mis) {
            $returnmess = array(
                'title' => urlencode("给车主发送服务通知"),
                'description' => urlencode('发送通知'),
                'picurl' => WL_URL_ARES . "images/notification_fill.png",
                'url' => app_url('app/distance', array(
                    'userid' => $user['id']
                ))
            );
            return $returnmess;
    ...
    static function store_rechange($openid, $user, $qrcode)
    {
        ...
        if ($mis) {
            $returnmess['recharge'] = array(
                'title' => urlencode("给车主充值"),
                'description' => urlencode('充值'),
                'picurl' => WL_URL_ARES . "images/notification_fill.png",
                'url' => app_url('app/distance/recharge', array(
                    'userid' => $user['id']
                ))
            );
            $returnmess['consume']  = array(
                'title' => urlencode("核销车主消费"),
                'description' => urlencode('核销'),
                'picurl' => WL_URL_ARES . "images/notification_fill.png",
                'url' => app_url('app/distance/consume', array(
                    'userid' => $user['id']
                ))
            );
            return $returnmess;
