...
class membercard
{
    static function get_cardid($userid, $sid)
    {
        ...
        if (empty($userid) || empty($sid))
            return FALSE;
        $data = pdo_get('weliam_shiftcar_membercard', array(
            'uniacid' => $_W['uniacid'],
            'mid' => $userid,
            'sid' => $sid
        ));
    ...
    {
        global $_W;
        if (empty($retype) || empty($fee) || empty($card) || empty($mid))
            return FALSE;
        if ($retype == 1) {
            $fee = Util::currency_format($fee);
            pdo_update('weliam_shiftcar_membercard', array(
                'credit2' => $fee + $card['credit2']
            ), array(
                'id' => $card['id']
            ));
            pdo_insert('weliam_shiftcar_mcrecord', array(
                'uniacid' => $_W['uniacid'],
                'mid' => $userid,
                'sid' => $sid,
                'type' => $retype,
    ...
    function mc_notice_recharge($openid, $uid = 0, $num = 0, $url = '', $remark = '')
    {
        ...
        $time   = date('Y-m-d H:i');
        if (empty($url)) {
            $url = murl('mc/bond/credits', array(
                'credittype' => 'credit2',
                'type' => 'record',
                'period' => '1'
            ), true, true);
