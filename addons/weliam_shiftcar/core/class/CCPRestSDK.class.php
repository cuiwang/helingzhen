...
class REST
{
    ...
    function subAuth()
    {
        if ($this->ServerIP == "") {
            ...
            $data->statusMsg = 'IP为空(云通信没设置好)';
          return $data;
        }
        if($this->ServerPort<=0){
            $data = new stdClass();
            $data->statusCode = '172005';
            $data->statusMsg = '端口错误（小于等于0）(云通信没设置好)';
          return $data;
        }
        if($this->SoftVersion==""){
            $data = new stdClass();
            $data->statusCode = '172013';
            $data->statusMsg = '版本号为空(云通信没设置好)';
          return $data;
        } 
        if($this->SubAccountSid==""){
            $data = new stdClass();
            $data->statusCode = '172008';
            $data->statusMsg = '子帐号为空(云通信没设置好)';
          return $data;
        }
        if($this->SubAccountToken==""){
            $data = new stdClass();
            $data->statusCode = '172009';
            $data->statusMsg = '子帐号令牌为空(云通信没设置好)';
          return $data;
        }
        if($this->AppId==""){
            $data = new stdClass();
            $data->statusCode = '172012';
            $data->statusMsg = '应用ID为空(云通信没设置好)';
          return $data;
        }  
   }
   ...
   function accAuth()
   {
       if($this->ServerIP==""){
        ...
        if($this->AccountSid==""){
            ...
            $data->statusMsg = '主帐号为空(云通信没设置好)';
          return $data;
        }
        if($this->AccountToken==""){
            $data = new stdClass();
            $data->statusCode = '172007';
            $data->statusMsg = '主帐号令牌为空(云通信没设置好)';
          return $data;
        }
        if($this->AppId==""){
            $data = new stdClass();
