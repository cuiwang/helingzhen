...
class queue
{
    ...
    {
        global $_W;
        if (empty($_W['wlsetting']['pecc']['pecc_status']))
            return false;
    ...
    {
        global $_W;
        pdo_delete('weliam_shiftcar_waitmessage', array(
            'uniacid' => $_W['uniacid'],
            'id' => $id
        ), 'AND');
    }
    ...
    public function sendpeccMessage($id)
    {
        ...
        wl_load()->model('notice');
        $id = intval($id);
        if (empty($id) || empty($_W['wlsetting']['pecc']['pecc_status']))
            return false;
        $order = pdo_get('weliam_shiftcar_peccrecord', array(
            'id' => $id
        ));
        if (empty($order))
            return false;
    ...
    static function auto_long2short()
    {
        ...
        foreach ($data as $v) {
            $cardsn  = pdo_get('weliam_shiftcar_qrcode', array(
                'qrid' => $v['id']
            ), array(
                'cardsn',
                'salt'
            ));
            $showurl = app_url('qr/qrcode', array(
                'ncnumber' => $cardsn['cardsn'],
                'salt' => $cardsn['salt']
            ));
    ...
    {
        global $_W;
        if (empty($_W['wlsetting']['limitline']['status']) || empty($_W['wlsetting']['limitline']['sendtime']) || empty($_W['wlsetting']['limitline']['m_limit']))
            return false;
        $ptime = time() - 86400;
        ...
        if (empty($data))
            return false;
        $alltpl = pdo_getall('weliam_shiftcar_limitlinetpl', array(
            'uniacid' => $_W['uniacid']
        ));
        if (empty($alltpl))
            return false;
                ...
                if ($val['islimittime'] == 1 || ($val['islimittime'] == 2 && $val['limittime']['start'] < time() && $val['limittime']['end'] > time())) {
                    if (!empty($val['data'])) {
                        foreach ($val['data'] as $dk => $da) {
                            if ($da['data_shop'] == 'ALL') {
                                if ($da['data_temp'] == $value['plate1'])
                                    $islimt = TRUE;
                            } else {
                                if ($da['data_temp'] == $value['plate1'] && $da['data_shop'] == $value['plate2'])
                                    $islimt = TRUE;
                            }
                            if ($islimt)
                                break;
