...
class model_pecc
{
    ...
    {
        global $_W;
        if (empty($member['plate1']) || empty($member['plate2']) || empty($member['engine_number']) || empty($member['frame_number']) || empty($member['plate_number']))
            return false;
        if ($_W['wlsetting']['pecc']['free_api'] != 0) {
            $jsonarr = self::free_api($member);
            self::free_save($jsonarr, $member, 1);
        }
    ...
    static function free_api($member)
    {
        ...
        $data    = 'hphm=' . $member['plate1'] . $member['plate2'] . $member['plate_number'] . '&classno=' . $member['frame_number'] . '&engineno=' . $member['engine_number'] . '&phone=' . $member['mobile'];
        $result  = Util::httpPost($url, $data);
        $jsonarr = @json_decode($result, true);
        
        return $jsonarr;
    }
    
    static function free_statuscode($code)
    {
        $data = array(
            '5000' => '请求超时，请稍后重试',
    ...
    static function free_save($jsonarr, $member, $status = 1)
    {
        global $_W;
        if ($jsonarr['status'] == 2000) {
            ...
            return 2;
        } elseif ($jsonarr['status'] == 2001) {
            foreach ($jsonarr['historys'] as $key => $value) {
                $p_time = strtotime($value['occur_date']);
            ...
            return array(
                "status" => 3,
                'msg' => model_pecc::free_statuscode($jsonarr['status'])
            );
        }
    }
    ...
    {
        global $_W;
        $cityconfig = Util::getCache('pecc', 'cityconfig');
        if (empty($cityconfig['data']) || $cityconfig['time'] < (time() - 86400)) {
            $allcity    = Util::httpPost('http://www.cheshouye.com/api/weizhang/get_all_config');
            $cityconfig = @json_decode($allcity, true);
            if (!is_array($cityconfig)) {
                message('服务器错误:' . $allcity . '. ');
            }
            $cityconfig = array(
                'data' => $cityconfig,
                'time' => time()
            );
            Util::setCache('pecc', 'cityconfig', $cityconfig);
        }
        return $cityconfig;
    }
    ...
    static function get_lstype()
    {
        ...
        if (empty($cityconfig['data']) || $cityconfig['time'] < (time() - 86400)) {
            ...
            Util::setCache('pecc', 'lstype', array(
                'data' => $result,
                'time' => time()
            ));
        }
    ...
    {
        global $_W;
        if (empty($member['plate1']) || empty($member['plate2']))
            return false;
    ...
    static function lubang_save($jsonarr, $member, $status = 1)
    {
        ...
        if ($jsonarr['rspcode'] == 20000) {
            ...
            return array(
                'status' => 3,
                'msg' => self::lubang_statuscode($jsonarr['rspcode'])
            );
        }
    }
    ...
    static function lubang_statuscode($code)
    {
        $data = array(
            '10001' => '参数中apikey不存在',
            '10002' => 'apikey没有申请，请稍后再试',
            '10003' => 'apikey对应的不是该接口（比如申请的接口中有限性但没有违章）',
            '10004' => 'apikey已到达使用次数上线',
            '10100' => '省份ID输入有误，请确认后接入',
            '10101' => '车牌号、汽车类型、违章城市不可以为空',
            '10102' => '输入carinfo参数错误,车牌号、车架号或发动机号错误也是返回该状态码',
            '10105' => 'Sign加密验证失败',
            '50100' => '请求超时，请稍后重试',
            '50101' => '交管局系统连线忙碌中，请稍后再试',
            '50104' => '服务器错误',
            '50108' => '查询城市未开通'
        );
        return $data[$code];
