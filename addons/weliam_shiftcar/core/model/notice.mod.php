...
function movecar_notice($openid, $recordid)
{
    ...
    $url      = app_url('member/record/detail', array(
        'id' => $recordid
    ));
...
function sendlimit_notice($str)
{
    ...
    $tpl      = pdo_get('weliam_shiftcar_limitlinetpl', array(
        'uniacid' => $_W['uniacid'],
        'id' => $str['tplid']
    ));
    ...
    $url      = app_url('app/limitline', array(
        'id' => $tpl['id']
    ));
...
{
    global $_W;
    $hide   = pdo_get('weliam_shiftcar_hidenotice', array(
        'id' => $str
    ), array(
        'touser',
        'address'
    ...
    if (is_array($touser) && !empty($touser)) {
        foreach ($touser as $key => $value) {
            ...
            $postdata = array(
                ...
                "keyword4" => array(
                    "value" => $hide['address'] . '附近，有交警贴罚单，请您尽快离开！',
                    "color" => "#173177"
                )
            );
            $url      = '';
