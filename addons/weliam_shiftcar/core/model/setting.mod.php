...
function wlsetting_load()
{
    ...
    if (empty($_W['wlsetting'])) {
        $settings = pdo_fetch_many('weliam_shiftcar_setting', array(
            'uniacid' => $_W['uniacid']
        ), array(
            'key',
            'value'
...
function wlsetting_save($data, $key)
{
    ...
    $exists          = pdo_select_count('weliam_shiftcar_setting', array(
        'key' => $key,
        'uniacid' => $_W['uniacid']
    ));
    if ($exists) {
        $return = pdo_update('weliam_shiftcar_setting', $record, array(
            'key' => $key,
            'uniacid' => $_W['uniacid']
        ));
    } else {
...
{
    global $_W;
    $settings = pdo_fetch_one('weliam_shiftcar_setting', array(
        'key' => $key,
        'uniacid' => $_W['uniacid']
    ), array(
        'value'
