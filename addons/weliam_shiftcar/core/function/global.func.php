...
{
    if ($data) {
        pdo_insert('core_text', array(
            'content' => iserializer($data)
        ));
...
function puv()
{
    ...
    $puv = pdo_getcolumn('weliam_shiftcar_puv', array(
        'uniacid' => $_W['uniacid'],
        'date' => date('Ymd')
    ), 'id');
    if (empty($puv)) {
        pdo_insert('weliam_shiftcar_puv', array(
            'uniacid' => $_W['uniacid'],
            'pv' => 0,
            'uv' => 0,
            'date' => date('Ymd')
    ...
    if ($_W['mid']) {
        $myp = pdo_getcolumn('weliam_shiftcar_puvrecord', array(
            'uniacid' => $_W['uniacid'],
            'date' => date('Ymd'),
            'mid' => $_W['mid']
        ), 'id');
        if (empty($myp)) {
            ...
            pdo_insert('weliam_shiftcar_puvrecord', array(
                'uniacid' => $_W['uniacid'],
                'pv' => 0,
                'mid' => $_W['mid'],
                'date' => date('Ymd')
...
function returnnum_array($brands, $i)
{
    ...
    return array(
        $brandlist,
        array_slice($brands, $bgnum, $count)
    );
}
...
function keyExist($key = '')
{
    ...
    $keyword = pdo_fetch('SELECT rid FROM ' . tablename('rule_keyword') . ' WHERE content=:content and uniacid=:uniacid limit 1 ', array(
        ':content' => trim($key),
        ':uniacid' => $_W['uniacid']
    ));
    
    if (!empty($keyword)) {
        $rule = pdo_fetch('SELECT * FROM ' . tablename('rule') . ' WHERE id=:id and uniacid=:uniacid limit 1 ', array(
            ':id' => $keyword['rid'],
            ':uniacid' => $_W['uniacid']
        ));
