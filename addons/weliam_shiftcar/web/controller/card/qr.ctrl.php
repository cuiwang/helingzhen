...
if ($op == 'summary') {
    //饼状图1
    $usednum    = pdo_fetchcolumn("SELECT COUNT(id) FROM " . tablename('weliam_shiftcar_qrcode') . " WHERE uniacid = '{$_W['uniacid']}' and status = 2");
    $invalidnum = pdo_fetchcolumn("SELECT COUNT(id) FROM " . tablename('weliam_shiftcar_qrcode') . " WHERE uniacid = '{$_W['uniacid']}' and status = 3");
    $notusenum  = pdo_fetchcolumn("SELECT COUNT(id) FROM " . tablename('weliam_shiftcar_qrcode') . " WHERE uniacid = '{$_W['uniacid']}' and status = 1");
    //饼状图2
    ...
    foreach ($remark_arr as $key => $item) {
        $arr2[] = pdo_fetchcolumn("SELECT COUNT(id) FROM " . tablename('weliam_shiftcar_qrcode') . " WHERE uniacid = '{$_W['uniacid']}' and remark = '{$item}' ");
        $arr3[] = pdo_fetchcolumn("SELECT COUNT(id) FROM " . tablename('weliam_shiftcar_qrcode') . " WHERE uniacid = '{$_W['uniacid']}' and remark = '{$item}' and status = 2 ");
    }
    ...
    if (!empty($list)) {
        foreach ($list as $index => &$qrcode) {
            ...
            if ($qrcode['model'] == 2) {
                ...
                $qrcode['showurl']    = app_url('qr/qrcode/show', array(
                    'ncnumber' => $qrcode['cardsn'],
                    'salt' => $qrcode['salt']
                ));
        ...
        foreach ($list as $k => $v) {
            ...
            if ($wq_qr['model'] == 3) {
                ...
                if ($str1 == 'http://w.url.cn') {
                    ...
                    $v['showurl'] = app_url('qr/qrcode', array(
                        'ncnumber' => $v['cardsn'],
                        'salt' => $v['salt']
                    ));
                }
...
if ($op == 'del') {
    if ($_GPC['scgq']) {
        $list = pdo_fetchall("SELECT id FROM " . tablename('qrcode') . " WHERE uniacid = :uniacid AND acid = :acid AND status = '0' AND type='scene'", array(
            ':uniacid' => $_W['uniacid'],
            ':acid' => $_W['acid']
        ), 'id');
        if (!empty($list)) {
...
if ($op == 'post') {
    ...
    if (pdo_tableexists('wlmerchant_merchantdata')) {
        $remark_arr = pdo_getall('wlmerchant_merchantdata', array(
            'uniacid' => $_W['uniacid']
        ), array(
            'id',
            'storename'
...
if ($op == 'get') {
    ...
    $storeid = intval($_GPC['storeid']);
    $acid    = intval($_W['acid']);
    $qrcid   = pdo_fetchcolumn("SELECT COUNT(id) FROM " . tablename('weliam_shiftcar_qrcode') . " WHERE uniacid = :uniacid AND model in(2,3) ", array(
        ':uniacid' => $_W['uniacid']
    ));
    ...
    if ($qrctype == 3) {
        $scene_str = 'PT' . $sstr;
        $is_exist  = pdo_fetchcolumn('SELECT id FROM ' . tablename('qrcode') . ' WHERE uniacid = :uniacid AND acid = :acid AND scene_str = :scene_str AND model = 3', array(
            ':uniacid' => $_W['uniacid'],
            ':acid' => $_W['acid'],
            ':scene_str' => $scene_str
        ));
        ...
        $result['url']                                = app_url('qr/qrcode', array(
            'ncnumber' => 'NC' . $addressid . sprintf("%07d", $sstr)
        ));
    } else if ($qrctype == 2) {
        $uniacccount = WeAccount::create($acid);
        $scene_str   = 'YJ' . $sstr;
        $is_exist    = pdo_fetchcolumn('SELECT id FROM ' . tablename('qrcode') . ' WHERE uniacid = :uniacid AND acid = :acid AND scene_str = :scene_str AND model = 2', array(
            ':uniacid' => $_W['uniacid'],
            ':acid' => $_W['acid'],
            ':scene_str' => $scene_str
        ));
...
if ($op == 'extend') {
    ...
    $id = intval($_GPC['id']);
    if (!empty($id)) {
        $qrcrow = pdo_fetch("SELECT * FROM " . tablename('qrcode') . " WHERE uniacid = :uniacid AND id = :id LIMIT 1", array(
            ':uniacid' => $_W['uniacid'],
            ':id' => $id
        ));
        $update = array();
        if ($qrcrow['model'] == 1) {
            ...
            pdo_update('qrcode', $update, array(
                'id' => $id,
                'uniacid' => $_W['uniacid']
            ));
        }
...
if ($op == 'display') {
    ...
    if (!empty($list)) {
        ...
        foreach ($list as $index => &$qrcode) {
            ...
            $list[$index]['mid']    = pdo_getcolumn('weliam_shiftcar_member', array(
                'openid' => $qrcode['openid'],
                'uniacid' => $_W['uniacid']
            ), 'id');
            $list[$index]['cardsn'] = pdo_getcolumn('weliam_shiftcar_qrcode', array(
                'qrid' => $qrcode['qid']
            ), 'cardsn');
        }
...
if ($op == 'delsata') {
    $id = $_GPC['id'];
    $b  = pdo_delete('qrcode_stat', array(
        'id' => $id,
        'uniacid' => $_W['uniacid']
    ));
    if ($b) {
...
if ($op == 'dosalt') {
    if ($_W['ispost']) {
        ...
        pdo_update('weliam_shiftcar_qrcode', $data, array(
            'id' => $qrcode['id']
        ));
        die(json_encode(array(
            'result' => 1
...
if ($op == 'remark') {
    if (checksubmit('submit')) {
        ...
        for ($i = $cardnumber[0]; $i <= $cardnumber[1]; $i++) {
            ...
            pdo_update('weliam_shiftcar_qrcode', array(
                'remark' => $remark
            ), array(
                'uniacid' => $_W['uniacid'],
                'cardsn' => $movecode
