...
if ($op == 'display') {
    ...
    while ($endtime >= $starttime) {
        $allsnum   = pdo_fetchcolumn("SELECT COUNT(id) FROM " . tablename('weliam_shiftcar_record') . " WHERE createtime >= :createtime AND createtime <= :endtime", array(
            ':createtime' => $endtime - 86400,
            ':endtime' => $endtime
        ));
        $allapinum = pdo_fetchcolumn("SELECT COUNT(id) FROM " . tablename('weliam_shiftcar_apirecord') . " WHERE createtime >= :createtime AND createtime <= :endtime", array(
            ':createtime' => $endtime - 86400,
            ':endtime' => $endtime
        ));
        $type1     = pdo_fetchcolumn("SELECT COUNT(id) FROM " . tablename('weliam_shiftcar_apirecord') . " WHERE type = 1 AND createtime >= :createtime AND createtime <= :endtime", array(
            ':createtime' => $endtime - 86400,
            ':endtime' => $endtime
        ));
        $type2     = pdo_fetchcolumn("SELECT COUNT(id) FROM " . tablename('weliam_shiftcar_apirecord') . " WHERE type = 2 AND createtime >= :createtime AND createtime <= :endtime", array(
            ':createtime' => $endtime - 86400,
            ':endtime' => $endtime
        ));
        $type3     = pdo_fetchcolumn("SELECT COUNT(id) FROM " . tablename('weliam_shiftcar_apirecord') . " WHERE type = 3 AND createtime >= :createtime AND createtime <= :endtime", array(
            ':createtime' => $endtime - 86400,
            ':endtime' => $endtime
        ));
        $type4     = pdo_fetchcolumn("SELECT COUNT(id) FROM " . tablename('weliam_shiftcar_apirecord') . " WHERE type = 4 AND createtime >= :createtime AND createtime <= :endtime", array(
            ':createtime' => $endtime - 86400,
            ':endtime' => $endtime
        ));
