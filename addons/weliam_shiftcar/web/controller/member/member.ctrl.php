...
if ($op == 'display') {
    ...
    foreach ($list as $key => $value) {
        ...
        if ($value['ncnumber']) {
            $qrid = pdo_get('weliam_shiftcar_qrcode', array(
                'cardsn' => $value['ncnumber'],
                'uniacid' => $_W['uniacid']
            ), array(
                'qrid',
                'model'
...
if ($op == 'detail') {
    ...
    $list     = pdo_pagination($sqlTotal, $sqlData, $params, '', $total, $page, $size);
    foreach ($list as $key => $value) {
        $send_member             = pdo_get('weliam_shiftcar_member', array(
            'id' => $value['sendmid']
        ), array(
            'avatar',
            'nickname'
        ));
        $take_member             = pdo_get('weliam_shiftcar_member', array(
            'id' => $value['takemid']
        ), array(
            'avatar',
            'nickname',
...
if ($op == 'summary') {
    ...
    $maxnum = max($address_arr['beijing'], $address_arr['tianjing'], $address_arr['shanghai'], $address_arr['chongqing'], $address_arr['hebei'], $address_arr['henan'], $address_arr['yunnan'], $address_arr['liaoning'], $address_arr['heilongjiang'], $address_arr['hunan'], $address_arr['anhui'], $address_arr['shandong'], $address_arr['xingjiang'], $address_arr['jiangshu'], $address_arr['zhejiang'], $address_arr['jiangxi'], $address_arr['hubei'], $address_arr['guangxi'], $address_arr['ganshu'], $address_arr['shanxi'], $address_arr['neimenggu'], $address_arr['sanxi'], $address_arr['jiling'], $address_arr['fujian'], $address_arr['guizhou'], $address_arr['guangdong'], $address_arr['qinghai'], $address_arr['xizhang'], $address_arr['shichuan'], $address_arr['ningxia'], $address_arr['hainan']) + 5;
}
