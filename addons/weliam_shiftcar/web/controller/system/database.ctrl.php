...
if ($op == 'repair') {
    $waitdo    = 0;
    $carmember = pdo_getall('weliam_shiftcar_member', array(
        'status' => 2
    ), array(
        'uniacid',
        'ncnumber',
    ...
    foreach ($carmember as $key => $value) {
        $qrcode = pdo_getall('weliam_shiftcar_qrcode', array(
            'mid' => $value['id']
        ), array(
            'uniacid',
            'id'
...
if ($op == 'detail') {
    ...
    $parentid  = $_GPC['id'];
    $brand     = $_GPC['brand'];
    $categorys = pdo_getall('weliam_shiftcar_class', array(
        'brandid' => $parentid
    ));
    foreach ($categorys as $key => $value) {
        $childrens                   = pdo_getall('weliam_shiftcar_sclass', array(
            'classid' => $value['id']
        ));
...
if ($op == 'addclass') {
    ...
    if (checksubmit('submit')) {
        $name = $_GPC['name'];
        if ($_GPC['id']) {
            $res = pdo_update('weliam_shiftcar_class', array(
                'name' => $name
            ), array(
                'id' => $_GPC['id']
            ));
        } else {
            $res = pdo_insert('weliam_shiftcar_class', array(
                'name' => $name,
                'brandid' => $brandid
            ));
        }
        if ($res) {
            message('保存成功', web_url('system/database/detail', array(
                'id' => $parentid
            )), 'success');
        } else {
            message('保存失败', web_url('system/database/detail', array(
                'id' => $parentid
            )), 'error');
        }
    }
...
if ($op == 'deleteclass') {
    ...
    $parentid = $_GPC['parentid'];
    $id       = $_GPC['id'];
    $res      = pdo_delete('weliam_shiftcar_class', array(
        'id' => $id
    ));
    if ($res) {
        message('删除成功', web_url('system/database/detail', array(
            'id' => $parentid
        )), 'success');
    } else {
        message('删除失败', web_url('system/database/detail', array(
            'id' => $parentid
        )), 'error');
    }
}
if ($op == 'addsclass') {
    ...
    if (checksubmit('submit')) {
        $name = $_GPC['name'];
        if ($_GPC['id']) {
            $res = pdo_update('weliam_shiftcar_sclass', array(
                'name' => $name
            ), array(
                'id' => $_GPC['id']
            ));
        } else {
            $res = pdo_insert('weliam_shiftcar_sclass', array(
                'name' => $name,
                'classid' => $classid
            ));
        }
...
if ($op == 'deletesclass') {
    ...
    $parentid = $_GPC['parentid'];
    $id       = $_GPC['id'];
    $res      = pdo_delete('weliam_shiftcar_sclass', array(
        'id' => $id
    ));
    if ($res) {
        message('删除成功', web_url('system/database/detail', array(
