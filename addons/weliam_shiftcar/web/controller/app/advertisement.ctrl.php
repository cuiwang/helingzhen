...
if ($op == 'dele') {
    $id     = intval($_GPC['id']);
    $result = pdo_delete("weliam_shiftcar_advertisement", array(
        'id' => $id
    ));
    if (empty($result)) {
...
if ($op == 'post') {
    $id = intval($_GPC['id']);
    if ($id) {
        $item            = pdo_get('weliam_shiftcar_advertisement', array(
            'id' => $id
        ));
    ...
    if (checksubmit('submit')) {
        ...
        if ($_GPC['advtype'] == 1 && empty($_GPC['cardnumber']))
            message('请填写挪车卡号码段');
        if ($_GPC['advtype'] == 2 && empty($_GPC['remark']))
            message('请选择挪车卡场景备注');
        if (!empty($_GPC['time'])) {
        ...
        if ($id) {
            pdo_update('weliam_shiftcar_advertisement', $base, array(
                'id' => $id
            ));
        } else {
