...
if ($op == 'dele') {
    $id     = intval($_GPC['id']);
    $result = pdo_delete("weliam_shiftcar_limitlinetpl", array(
        'id' => $id
    ));
    if (empty($result)) {
...
if ($op == 'post') {
    $id = intval($_GPC['id']);
    if ($id) {
        $item              = pdo_get('weliam_shiftcar_limitlinetpl', array(
            'id' => $id
        ));
    ...
    if (checksubmit('submit')) {
        ...
        $base = array(
            ...
            'interval' => serialize(array(
                'start' => $_GPC['intervalstart'],
                'end' => $_GPC['intervalend']
            )),
            'createtime' => time(),
        ...
        if ($id) {
            pdo_update('weliam_shiftcar_limitlinetpl', $base, array(
                'id' => $id
            ));
        } else {
...
if ($op == 'sync') {
    $share = pdo_getall('weliam_shiftcar_limitlinetpl', array(
        'isshare' => 1,
        'shareid' => 0
    ));
    if (empty($share)) {
    ...
    foreach ($share as $key => $value) {
        $haves = pdo_getcolumn('weliam_shiftcar_limitlinetpl', array(
            'uniacid' => $_W['uniacid'],
            'shareid' => $value['id']
        ), 'id');
