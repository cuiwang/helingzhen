...
if ($op == 'list') {
    ...
    foreach ($list as $key => $value) {
        $list[$key]['avatar']    = pdo_getcolumn('weliam_shiftcar_member', array(
            'id' => $value['mid']
        ), 'avatar');
            ...
            if ($ftype == 'xls') {
                ...
                for ($j = 2; $j <= $highestRow; $j++) {
                    ...
                    if (!empty($expressOrder) && !empty($expressName) && !empty($orderNo)) {
                        $res = pdo_update('weliam_shiftcar_apply', array(
                            'status' => 2,
                            'express' => $expressName,
                            'expresssn' => $expressOrder,
                            'sendtime' => TIMESTAMP
                        ), array(
                            'ordersn' => $orderNo
                        ));
                        if ($res) {
                            $mid    = pdo_getcolumn('weliam_shiftcar_apply', array(
                                'ordersn' => $orderNo
                            ), 'mid');
                            $openid = pdo_getcolumn('weliam_shiftcar_member', array(
                                'id' => $mid
                            ), 'openid');
...
if ($op == 'delete') {
    if (!empty($_GPC['id'])) {
        $id     = $_GPC['id'];
        $data   = pdo_fetch("SELECT * FROM " . tablename('weliam_shiftcar_apply') . " WHERE id = :id", array(
            ':id' => $id
        ));
        $data   = serialize($data);
        $result = pdo_delete('weliam_shiftcar_apply', array(
            'id' => $id
        ));
        if ($result) {
...
if ($op == 'confirmsend') {
    ...
    if (!empty($id)) {
        ...
        if ($res) {
            $mid    = pdo_get('weliam_shiftcar_apply', array(
                'id' => $id
            ), array(
                'mid',
                'ordersn'
