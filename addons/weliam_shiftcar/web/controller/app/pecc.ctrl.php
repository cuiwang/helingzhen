...
if ($op == 'display') {
    ...
    if (!empty($list)) {
        foreach ($list as $index => &$qrcode) {
            $wq_qr              = pdo_get('weliam_shiftcar_member', array(
                'id' => $qrcode['mid']
            ), array(
                'avatar',
                'nickname',
            ...
            $fanid              = pdo_getcolumn('mc_mapping_fans', array(
                'openid' => $wq_qr['openid']
            ), 'fanid');
...
if ($op == 'dele') {
    $id     = intval($_GPC['id']);
    $result = pdo_delete("weliam_shiftcar_peccrecord", array(
        'id' => $id
    ));
    if (empty($result)) {
...
if ($op == 'setting') {
    ...
    $smses    = pdo_getall('weliam_shiftcar_smstpl', array(
        'uniacid' => $_W['uniacid']
    ), array(
        'id',
        'name'
