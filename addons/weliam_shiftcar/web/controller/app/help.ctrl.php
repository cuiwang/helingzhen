...
if ($op == 'answer_list') {
    //解答列表
    $category_list = pdo_getall('weliam_shifcar_category', array(
        'uniacid' => $_W['uniacid'],
        'is_show' => 2
    ));
    ...
    foreach ($list as $key => $value) {
        $category                    = pdo_get('weliam_shifcar_category', array(
            'uniacid' => $_W['uniacid'],
            'id' => $value['categoryid']
        ));
...
if ($op == 'answer_edit') {
    ...
    if (!empty($answerid)) {
        $list = pdo_get('weliam_shifcar_question', array(
            'uniacid' => $_W['uniacid'],
            'id' => $answerid
        ));
    }
    ...
    if (checksubmit('submit')) {
        ...
        if (empty($answerid)) {
            ...
            pdo_update('weliam_shifcar_question', $data, array(
                'id' => $answerid
            ));
...
if ($op == 'answer_delete') {
    ...
    if (pdo_delete('weliam_shifcar_question', array(
        'uniacid' => $_W['uniacid'],
        'id' => $answerid
    ))) {
...
if ($op == 'category_list') {
    //解答分类
    $list           = pdo_fetchall("select * from" . tablename('weliam_shifcar_category') . "where uniacid=:uniacid order by sort desc,createtime desc", array(
        ':uniacid' => $_W['uniacid']
    ));
    $all_answer_num = 0;
    $all_scan       = 0;
    foreach ($list as $key => $value) {
        $answer_num               = pdo_fetch("select count(id),sum(scan) from" . tablename('weliam_shifcar_question') . " where uniacid=:uniacid and categoryid=:categoryid", array(
            ':uniacid' => $_W['uniacid'],
            ':categoryid' => $value['id']
        ));
...
if ($op == 'category_edit') {
    ...
    if (!empty($categoryid)) {
        $list = pdo_get('weliam_shifcar_category', array(
            'uniacid' => $_W['uniacid'],
            'id' => $categoryid
        ));
    }
    if (checksubmit('submit')) {
        ...
        if (empty($categoryid)) {
            ...
            pdo_update('weliam_shifcar_category', $data, array(
                'id' => $categoryid
            ));
...
if ($op == 'category_delete') {
    ...
    if (pdo_delete('weliam_shifcar_category', array(
        'id' => $categoryid,
        'uniacid' => $_W['uniacid']
    ))) {
        pdo_delete('weliam_shifcar_question', array(
...
if ($op == 'change_status') {
    $categoryid = $_GPC['categoryid'];
    $re         = pdo_get('weliam_shifcar_category', array(
        'id' => $categoryid
    ));
    $is_show    = $re['is_show'] == 1 ? 2 : 1;
    pdo_update('weliam_shifcar_category', array(
        'is_show' => $is_show
    ), array(
        'id' => $categoryid
    ));
...
if ($op == 'change_im') {
    //确定是否是热点
    $answerid     = $_GPC['answerid'];
    $re           = pdo_get('weliam_shifcar_question', array(
        'id' => $answerid
    ));
    $is_importent = $re['is_importent'] == 2 ? 1 : 2;
    pdo_update('weliam_shifcar_question', array(
        'is_importent' => $is_importent
    ), array(
        'id' => $answerid
    ));
