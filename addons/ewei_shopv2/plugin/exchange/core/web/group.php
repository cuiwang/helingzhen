		...
		else 
		{
			...
			if (!(empty($id))) 
			{
				...
				if ($goods['goods'] != false) 
				{
					...
					{
						$all[$k] = pdo_fetchall('SELECT * FROM ' . tablename('ewei_shop_goods') . ' WHERE uniacid = :uniacid AND deleted = 0 ' . "\r\r" . '        AND hasoption = 0 AND id = :id', array(':id' => $v, ':uniacid' => $_W['uniacid']));
					}
				}
