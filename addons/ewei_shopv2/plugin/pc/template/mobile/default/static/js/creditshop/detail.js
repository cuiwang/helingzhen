define(['core', 'tpl'], function (core, tpl) {
    var modal = {goods: false, logid: 0, realname: 0, mobile: 0, settime: 0};
        ...
        $("#openActionSheet").off("click").on("click", function () {
            if ($(this).parent().hasClass("btn-danger")) {
                if (modal.optionid == 0 && goods.hasoption == 1) {
                    modal.optionPicker();
                    core.json('pc/creditshop/detail/option', {goodsid: modal.goods.id}, function (ret) {
                        if (ret.status == 0) {
                ...
                location.href = core.getUrl('pc/creditshop/create', {id: goods.id, optionid: modal.optionid})
            } else {
    ...
    modal.getListlog = function (page, goodsid) {
        core.json('pc/creditshop/detail/getlistlog', {page: page, goodsid: goodsid}, function (ret) {
    ...
    modal.getListreply = function (page, goodsid) {
        core.json('pc/creditshop/detail/getlistreply', {page: page, goodsid: goodsid}, function (ret) {
            var result = ret.result;
            if (result.total <= 0) {
                $(".replymore").hide()
            } else {
    ...
    modal.pay = function () {
        core.json('pc/creditshop/detail/pay', {
            id: modal.goods.id,
    ...
    modal.lottery = function (goods) {
        var type = goods.type;
        if (type == 0) {
            core.json('pc/creditshop/detail/lottery', {id: modal.goods.id, logid: modal.logid}, function (json) {
                var result = json.result;
                if (result.status == -1) {
                    FoxUI.toast.show(json.result.message);
                    return
