(function (factory) {
    ...
    "use strict";
    var dragEl, ghostEl, rootEl, nextEl, lastEl, lastCSS, lastRect, activeGroup, tapEvt, touchEvt,
        expando = 'Sortable' + (new Date).getTime(), win = window, document = win.document, parseInt = win.parseInt,
        supportIEdnd = !!document.createElement('div').dragDrop, _silent = false,
    ...
    Sortable.prototype = {
        constructor: Sortable, _applyEffects: function () {
            ...
            if (target && !dragEl && (target.parentNode === el)) {
                ...
                if (touch) {
                    tapEvt = {target: target, clientX: touch.clientX, clientY: touch.clientY};
            ...
            if (touchEvt) {
                ...
                var target = document.elementFromPoint(touchEvt.clientX, touchEvt.clientY), parent = target,
            ...
            if (!_silent && (activeGroup === this.options.group) && (evt.rootEl === void 0 || evt.rootEl === this.el)) {
                ...
                if (el.children.length === 0 || el.children[0] === ghostEl || (el === evt.target) && _ghostInBottom(el, evt)) {
                    ...
                    var rect = lastRect, width = rect.right - rect.left, height = rect.bottom - rect.top,
    ...
    function _generateId(el) {
        var str = el.innerHTML + el.className + el.src, i = str.length, sum = 0;
        while (i--) {
