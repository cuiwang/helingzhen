...
class WxPayApi
{
	public static function redPacketPay($input, $timeOut = 6) {	
		$xml = $input->ToXml();
	...
	public static function enterprisePay($input, $timeOut = 6) {	
		$xml = $input->ToXml();
	 ...
	 * @return 成功时返回，其他抛异常
	 */
	public static function unifiedOrder($inputObj, $token, $timeOut = 6)
	{
		...
		$wechat = M('wechat') -> where("token = '%s'",$token) -> find();
		$wechat_conf = M('config') -> where("token = '%s'",$token) -> find();
		$inputObj->SetAppid($wechat['appid']);//公众账号ID
		$inputObj->SetMch_id($wechat_conf['mchid']);//商户号
		$inputObj->SetSpbill_create_ip($_SERVER['REMOTE_ADDR']);//终端ip	  
		//$inputObj->SetSpbill_create_ip("1.1.1.1");  	    
		$inputObj->SetNonce_str(self::getNonceStr());//随机字符串
		
		//签名
		$inputObj->SetSign($wechat_conf['mchkey']);
		$xml = $inputObj->ToXml();
		
		$startTimeStamp = self::getMillisecond();//请求开始时间
		$response = self::postXmlCurl($xml, $url, false, $timeOut,$token);
	...
	{		
		...
		if($useCert == true){
			...
			//$dir =str_replace( '\\' , '/' , realpath(dirname(__FILE__).'/../../../../')) . '\\Public\\Uploads\\pay\\' . $token;
