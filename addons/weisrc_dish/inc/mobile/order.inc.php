...
//已确认
//$order_list = pdo_fetchall("SELECT a.* FROM " . tablename($this->table_order) . " AS a LEFT JOIN " . tablename($this->table_stores) . " AS b ON a.storeid=b.id  WHERE a.weid = '{$weid}' AND a.status={$status} AND a.from_user='{$from_user}' ORDER BY a.id DESC LIMIT 20");
if ($_GPC['ispay'] == 1) {
