...
//        pdo_update($this->table_stores, array("is_show" => $_GPC['store_status']), array('id' => $storeid, 'weid' => $weid), 'AND');
...
//todayprice
    $today_order_price = pdo_fetchcolumn("SELECT sum(totalprice) FROM " . tablename($this->table_order) . " WHERE weid=:weid AND storeid=:storeid AND dateline>:time AND status=3 AND ispay=1 AND ismerge=0 ", array(':weid' => $this->_weid, ':storeid' => $storeid, ':time' => $zero_time));
 ...
 AND storeid=:storeid AND status=3 AND ispay=1 AND ismerge=0 ", array(':weid' => $this->_weid, ':storeid' => $storeid));
