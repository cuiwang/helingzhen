<?php
	...
	//下载图片到服务器开始
			...
			$filename1 = '/attachment/xuan_miaosha/storeimage/'.$_W['uniacid'].'/'.$openid."".time()."1.jpg";
			$filename2 = '/attachment/xuan_miaosha/storeimage/'.$_W['uniacid'].'/'.$openid."".time()."2.jpg";
			$filename3 = '/attachment/xuan_miaosha/storeimage/'.$_W['uniacid'].'/'.$openid."".time()."3.jpg";
			$length = $_GPC['length'];
			...
			if($length == 1){
				...
				mkdirs(IA_ROOT.'/attachment/xuan_miaosha/storeimage/'.$_W['uniacid']);
	...
	$result = pdo_insert("xuan_miaosha_showprize",$data);
	
	pdo_update('xuan_miaosha_period',array('status' => 7),array('period_number' => $period_number,'uniacid' => $_W['uniacid']));
