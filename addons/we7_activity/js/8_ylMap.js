	...
	$.fn.ylmap = function(options){
		...
		return this.each(function(){
			...
			*/
			...
			if(bigOpen) mapBoxPaent.find(".close_map").css('display','block');
			...
			*/
			var mapBox = $('<div id="BDMap" class="BDMap"></div>');			//地图容器Jquery对象
				...
				mapBoxPaent.append($('<div class="tit"><p><a href="javascript:void(0)"><span class="css_sprite01"></span>'+mapVal.address+'</a></p><p class="css_sprite01 close_map" href="javascript:void(0)"></p></div>'));
			...
			*/
			...
			var mapInit = function() {
				if(mapBoxPaent.size()>0){
					...
					map.addEventListener("touchmove", function(e){    		//地图点击窗口不关闭
						if(bigOpen){
							mapBoxPaent.css({
								'height':"100%"		
							})
							mapBoxPaent.find('tit').css({
								'bottom':0	
							})
						}
					});
			...
			*/
			...
			handleSuccess = function(position){
				...
				mapBoxPaent.find(".close_map").on('click',function(){
					...
					map.reset();
					map.centerAndZoom(point,15);
					if(!bigOpen) mapBoxPaent.find(".close_map").hide();
			...
			*/
			...
			function loadfunction() {  
				...
				var style_map =
						 ...
						 ".ylmap .tit p a span { position:absolute; left:15px; top:15px; display:inline-block; width:40px;height:40px; background-position: -288px 0; }"+
						 ".ylmap .tit .close_map { display:none; position: absolute; bottom: 15px; right: 20px; width: 40px; height: 40px; cursor:pointer; background-position: -288px -50px; }"+
