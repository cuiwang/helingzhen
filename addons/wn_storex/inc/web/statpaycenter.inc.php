...
$ops = array('detail', 'chart', 'display');
...
if ($op == 'display') {
	...
	$total = pdo_fetchcolumn('SELECT COUNT(*) FROM' . tablename('storex_paycenter_order') . $condition, $params);
...
if ($op == 'chart') {
	...
	$today_fee = floatval(pdo_fetchcolumn('SELECT SUM(final_fee) FROM ' . tablename('storex_paycenter_order') . ' WHERE uniacid = :uniacid AND status = 1 AND paytime >= :starttime AND paytime <= :endtime', array(':uniacid' => $_W['uniacid'], ':starttime' => $today_starttime, ':endtime' => $today_endtime)));
	...
	if ($_W['isajax'] && $_W['ispost']) {
		if ($type == 'date') {
			...
			$total = floatval(pdo_fetchcolumn('SELECT SUM(final_fee) FROM ' . tablename('storex_paycenter_order') . ' WHERE uniacid = :uniacid AND status = 1 AND paytime >= :starttime AND paytime <= :endtime', array(':uniacid' => $_W['uniacid'], ':starttime' => $starttime, ':endtime' => $endtime)));
			$out['total'] = $total;
