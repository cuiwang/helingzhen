...
if ($op == 'display') {
	...
	if (!empty ($hx_status) || $hx_status == '0') {
		$con .= " and hx_status=$hx_status";
	}
...
if ($op == 'post') {
	...
	if (checksubmit('submit')) {
		$data = array (
			...
			"cj_counter" => $_GPC['cj_counter'],
			"give_openid" => $_GPC['give_openid'],
			"is_give" => $_GPC['is_give'],
			"custom1"=>$_GPC['custom1'],	
			"custom2"=>$_GPC['custom2'],	
			"custom3"=>$_GPC['custom3'],	
			"is_pay"=>$_GPC['is_pay'],	
			"qrcode_poster"=>$_GPC['qrcode_poster'],
			"pay_money"=>$_GPC['pay_money'],	
			"is_return"=>$_GPC['is_return'],
		);
