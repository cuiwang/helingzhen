...
function  ewei_shop($userinfo,$child_userinfo){
  ...
  function update_parent($activity,$user=array()){
	...
	$openid=$_COOKIE["cgc_baoming_share_parent_".$uniacid."_".$id];
	...
	/* if ($activity['ewei_shop']){
	  $user=getFromUser("","cgc_baoming_share");
  ...
  //	if (!empty($activity['yq_mode'])){
  	  $url = $main_url . "app/index.php?i=".$_W['uniacid']."&j=".$_W['acid']."&c=entry&m=cgc_baoming_share&do=yq&pid=$from_user&id=$id";
      return $url;
  //	}
  ...
  function send_ali_sms($settings,$tel,$code,$product){
  	include IA_ROOT."/addons/cgc_baoming_share/ali_sms/TopSdk.php";
    $item="";
  ...
  function gbk_to_utf8($str){
   ...
   function getRandom_code($activity_id,$cj_code_start) {
    global $_W;
    
    $min=0;
    $max=9;
   $code_id= strval(mt_rand(1,$max)).strval(mt_rand($min,$max)).strval(mt_rand($min,$max))
   .strval(mt_rand($min,$max)).strval(mt_rand($min,$max)).strval(mt_rand($min,$max));
     return $code_id;
    $code_id = pdo_fetchcolumn('SELECT code_id FROM ' . tablename('bc_newbaoming_code') 
    ...
    if (empty($code_id)) {
      ...
      pdo_insert("bc_newbaoming_code", array('uniacid'=>$_W['uniacid'], 'code_id'=>$code_id,'activity_id'=>$activity_id,'createtime'=>time()));
      
    } else {
      WeUtility::logging('getNextAvaliableCode_id ok', $code_id);
      $code_id++;
      pdo_update("bc_newbaoming_code", array('code_id'=>$code_id,"createtime"=>time()), array('uniacid'=>$_W['uniacid'],'activity_id'=>$activity_id));
    } 
    return $code_id;
  }
...
function test($t){
	...
	$num = strlen($t);
	$str = substr($code_id,-$num);
	echo $num.'<br/>';
	echo $str.'<br/>';
	if($str==$t){//对比尾数
		$code_id = $code_id - mt_rand(1, 9);
		if(strlen($code_id)<6){
			$code_id=$code_id+100000;
		}
	}
	return $code_id;
}
...
function getNextAvaliableCjcode($activity_id, $cj_code_start, $activity = array())
{
	...
	$min = 0;
	$max = 9;
	if ($activity['whether_random']) {
		$code_id = strval(mt_rand(1, $max)) . strval(mt_rand($min, $max)) . strval(mt_rand($min, $max))
			. strval(mt_rand($min, $max)) . strval(mt_rand($min, $max)) . strval(mt_rand($min, $max));

		if ($activity['cj_code_forbidden']) {
			$num = strlen($activity['cj_code_forbidden']);
			$str = substr($code_id,-$num);
			if($str==$activity['cj_code_forbidden']){//对比尾数
				$code_id = $code_id - mt_rand(1, 9);
				if(strlen($code_id)<6){
					$code_id=$code_id+100000;
				}
		...
		/*if ($activity['cj_code_forbidden'] && strexists($code_id,$activity['cj_code_forbidden'])){
		  $code_id= strval(mt_rand(1,$max)).strval(mt_rand($min,$max)).strval(mt_rand($min,$max))
		 .strval(mt_rand($min,$max)).strval(mt_rand($min,$max)).strval(mt_rand($min,$max));
		}*/
		
		return !empty($activity['code_prefix'])?$activity['code_prefix'].str_pad($code_id,6,"0",STR_PAD_LEFT):$code_id;
	}
...
//授权获取用户信息。	
 function getParent($id) {
 	 global $_W,$_GPC;	
 	$uniacid = $_W['uniacid'];	
   if (!empty($_GPC['pid']) && empty($_COOKIE["cgc_baoming_share_parent_".$uniacid."_".$id])){ 	
   	  setcookie("cgc_baoming_share_parent_".$uniacid."_".$id,$_GPC['pid'], time()+3600*(24*5)); 
   	  return $_GPC['pid'];
   }	   
   return $_COOKIE["cgc_baoming_share_parent_".$uniacid."_".$id];
 
 }


//授权获取用户信息。	
 function getFromUser($settings,$modulename,$activity=array()) 
{
  global $_W,$_GPC;	
  $uniacid = $_W['uniacid'];
...
/* 	if ($settings['duli']){
      ...
      $activity = $cgc_baoming_activity->getOne($id); 
       $settings['appid']=empty($activity['appid'])? $settings['appid']:$activity['appid'];
 ...
 */
  if(empty($_COOKIE[$modulename."_user_".$uniacid])){ 	
   ...
   /*  if (!empty($secret)){
      ...
      if (!empty($activity['appid'])){
       $url.="&token2=12".$activity['appid'];
     }*/
