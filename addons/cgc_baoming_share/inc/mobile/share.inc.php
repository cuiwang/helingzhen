...
$user_json = getFromUser($settings, $modulename,$activity);
...
/*	  ini_set('display_errors', '1');
	...
	if (empty ($temp)) {		
	    ...
	    if (($user['id']) > 0 && $activity['activity_type']==2 && $activity['pay_time_point']==1) {
	       ...
	       if ($_GPC['direct']){
	         header("location:".$this->createMobileUrl("payment",array('id'=>$_GPC['id'],'tid'=>$params['tid'],'activity_id'=>$params['activity_id'],'title'=>$activity['title'])));
	         exit();
	       } 
	     ...
	     if ($_GPC['direct']){
	       header("location:".$this->createMobileUrl("share",array('op'=>"display",'id'=>$_GPC['id'])));
	       exit();
	     }
