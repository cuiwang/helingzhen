...
function get_post_data($id){
	...
	$pars[':activity_id'] = $id;
	$sql = 'SELECT * FROM ' . tablename('cgc_baoming_poster') . " WHERE `uniacid`=:uniacid AND `activity_id`=:activity_id";
	$poster = pdo_fetch($sql, $pars);
  	return $poster;
}
...
function gen_Image($param, $qr_file) {
	global $_W;
	load()->func('file');
	load()->func('logging');
	...
	$target_file = $path . '/qr-image-' . $param['openid'] . rand() . '.jpg';
	...
	$bg = imagecreates(tomedia($bg_file));
...
function mergeText($target, $data, $text) {
	$font = IA_ROOT . '/addons/cgc_baoming_share/msyhbd.ttf';
...
function createBarcode1($obj, $qrimg) {
	global $_W;
	$ret = array ();
...
function gen_qr($url) {
	global $_GPC;
	$dir = "../attachment/cgc_baoming_share/image";
	load()->func('file');
	mkdirs($dir);
