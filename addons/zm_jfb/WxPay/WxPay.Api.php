...
class WxPayApi
{
	 ...
	 * @return 成功时返回，其他抛异常
	 */
	public static function unifiedOrder($inputObj,$mch,$key, $timeOut = 6)
	{
	    global $_W;
		...
		$inputObj->SetAppid($_W['account']['key']);//公众账号ID
		$inputObj->SetMch_id($mch);//商户号
		...
		$result = WxPayResults::Init($response['content'],$key);
	...
	public static function orderQuery($inputObj, $timeOut = 6)
	{
		...
		$startTimeStamp = self::getMillisecond();//请求开始时间
		$response = self::postXmlCurl($xml, $url, false, $timeOut);
		$result = WxPayResults::Init($response['content']);
		self::reportCostTime($url, $startTimeStamp, $result);//上报请求花费时间
	...
	private static function postXmlCurl($xml, $url, $useCert = false, $second = 30)
	{
	   ...
	   $data =  ihttp_request($url,$xml,array('CURLOPT_HTTPHEADER' => array('Content-Type: text/xml; charset=utf-8')),$second);
