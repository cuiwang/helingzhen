...
(function ($) {
    ...
    function Scroller(elem, settings) {
        ...
        function generateWheelItems(i) {
            ...
            for (j in warr[i]) {
                ...
                html += '<div class="dw-li dw-v" data-val="' + j + '" style="height:' + hi + 'px;"><div class="dw-i">' + warr[i][j] + '</div></div>';
                l++;
            }
            html += '</div>';
            return html;
        ...
        */
        that.show = function (prevAnim) {
            ...
            html += (s.display != 'inline' ? '<div class="dwbc' + (s.button3 ? ' dwbc-p' : '') + '"><span class="dwbw dwb-c"><span class="dwb">' + s.cancelText + '</span></span>' + (s.button3 ? '<span class="dwbw dwb-n"><span class="dwb">' + s.button3Text + '</span></span>' : '') + '<span class="dwbw dwb-s"><span class="dwb">' + s.setText + '</span></span></div></div>' : 'dayu整理<div class="dwcc"></div>') + '</div></div></div>';
            dw = $(html);
    ...
    a.mobiscroll.themes.wp = {
        defaults: {
            ...
            height: 60,
            accent: "none",
            dateOrder: "yymmMMddDD"	//wp排序
        },
        init: function(g, t) {
            var k, y;
            a(".dw", g).addClass("wp-" + t.settings.accent);
            a(".dwwl", g).bind("touchstart mousedown DOMMouseScroll mousewheel",
            function() {
                k = !0;
